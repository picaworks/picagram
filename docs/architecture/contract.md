...
| `lib/dither-mask.ts` | Masks beyond Bayer: `blueNoiseMatrix` by void and cluster, `clusterMatrix` for a printed dot screen, `maskAt`, `thresholdMask`, and `ditherLevels`. A shift moves the pattern between frames while the tone holds. |
| `lib/pixels.ts` | `inkPixels` writes coverage as one palette color, and `createPlate` holds a picture prepared once so an animated effect only ever draws from it. |
| `lib/filter.ts` | `sobelEdges` for gradient magnitude, and `boxMean` for a local mean through a summed area table. Measurements, never a blur anyone sees. |
| `lib/chart-marks.ts` | `gridLines`, `svgLabel`, and `polarPoint`: the axes, tick labels, and polar geometry every chart repeats. |
| `lib/chart-time.ts` | `parseTime`, `timeTicks`, and `formatTime`, always in UTC so both shapes print the same labels. |
| `lib/braille-plot.ts` | `createBraillePlot`: dots and lines in braille space, for a chart's glyph look. |
| `lib/geo.ts` and `lib/geo-land.ts` | The world: `geoDecode` reads the generated coastline string, `geoVector`, `geoTurn`, and `geoCut` place and clip points on a globe, `geoEqualEarth` flattens one, `geoGraticule` rules it, and `geoRaster` turns rings into a land mask. `lib/geo-land.ts` is generated by `npm run geo` from the public domain Natural Earth source in `sources/geo/`, and is never edited by hand. |
