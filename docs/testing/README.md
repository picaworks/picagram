...
| Invariant | `test/invariants.test.ts`, `meta.test.ts`, `license.test.ts`, `budget.test.ts`, `generated.test.ts`, `compose.test.ts`, and `site.test.ts`; the list is in `invariants.md` | the hard constraints in `AGENTS.md` cannot be broken silently | `npm test` |
