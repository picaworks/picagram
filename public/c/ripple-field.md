...
> Rings spread from seeded drops on a still surface, dispersing as they travel and cancelling where they cross, dithered on the GPU.

Category: shaders. Tags: ripple, water, shader, webgl, background, dither, interference. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 5.1 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/ripple-field.json
```
...
|---|---|---|---|
| `speed` | number | `0.18` | How fast the rings travel. 0 holds the surface still. |
| `drops` | number | `4` | How many drop points disturb the surface at once. |
| `wavelength` | number | `0.09` | Ring spacing, as a fraction of the host's shorter side. |
| `decay` | number | `0.5` | How quickly a ring system fades over its life, from 0 (lingers) to 1 (gone early). |
| `pointer` | boolean | `true` | Whether the pointer raises one ring that follows it. |
| `levels` | number | `5` | Tone steps the surface is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
...
// Pica · Ripple Field · ripple-field
...
// registry/shaders/ripple-field/core.ts
...
// registry/shaders/ripple-field/index.tsx
...
`;
  var FALLBACK = `repeating-radial-gradient(circle at 42% 46%, color-mix(in srgb, ${cssVar("accent")} 30%, transparent) 0px, color-mix(in srgb, ${cssVar("accent")} 30%, transparent) 2px, transparent 2px, transparent 26px)`;
  function uniforms(p) {
    return { u_seed: p.seed, u_speed: p.speed, u_drops: p.drops, u_wavelength: p.wavelength, u_decay: p.decay, u_levels: p.levels };
  }
...
(function () {
  ...
  var instance = PicaRippleField.mount(host, take(window.PICA_PROPS || {}));
