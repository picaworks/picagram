...
Category: immersive. Tags: terrain, horizon, landscape, hidden-line, canvas. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 3.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/terrain-field.json
```
...
|---|---|---|---|
| `rows` | number | `40` | Profile lines in the stack, from a flat hairline at the horizon to the nearest ridge. |
| `columns` | number | `160` | Points sampled along each profile line. More columns draw a smoother ridge. |
| `scale` | number | `1.8` | Horizontal frequency of the height field: how many landforms fit across the frame. |
| `height` | number | `0.5` | The most a ridge may rise, as a fraction of the frame's height. |
| `perspective` | number | `0.5` | How strongly the rows crowd toward the horizon and grow toward the viewer. |
| `speed` | number | `0.15` | How fast the rows drift toward the viewer. 0 holds the terrain still. |
| `weight` | number | `1` | Stroke width of each profile line, in CSS pixels. |
| `fade` | number | `0.6` | How much the far rows dim toward the muted color. 0 draws every row at full strength. |
...
// Pica · Terrain Field · terrain-field
...
// registry/immersive/terrain-field/core.ts
...
// registry/immersive/terrain-field/index.tsx
...
<title>Terrain Field · Pica</title>
...
var PicaTerrainField = (() => {
  ...
  var RISE_CURVE = 1.3;
  var RISE_GAIN = 0.8;
  var DETAIL_FREQ = 2.31;
  var DETAIL_AMP = 0.3;
  var DETAIL_BIAS = 19.7;
  var FAR = 1e9;
  ...
  var mount = (host, initial = {}) => {
    ...
    function draw(t) {
      ...
      if (ctx && width > 0 && height > 0) {
        ...
        const sc = clamp(props.scale, 0.5, 6);
        const fade = clamp(props.fade, 0, 1);
        const phase = t / 1e3 * clamp(props.speed, 0, 1) * ROW_RATE;
        const wrap = Math.ceil(phase);
        const span = cols - 1;
        const step = span / Math.max(1, pxW - 1);
        for (let j = 0; j < count; j++) {
          ...
          for (let c = 0; c < cols; c++) {
            const x = c / span;
            const n = noise.noise2(x * sc, depth) + DETAIL_AMP * noise.noise2(x * sc * DETAIL_FREQ + DETAIL_BIAS, depth * DETAIL_FREQ);
            samples[c] = baseY - n * amp;
          }
          const ink = 1 - fade * (1 - u);
          const r = Math.round(mutedInk[0] + (fgInk[0] - mutedInk[0]) * ink);
...
(function () {
  ...
  var instance = PicaTerrainField.mount(host, take(window.PICA_PROPS || {}));
