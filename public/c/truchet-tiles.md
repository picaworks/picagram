...
> A seeded field of connected quarter circle paths or diagonal zigzags drawn behind content.

Category: patterns. Tags: truchet, tiling, maze, svg, seeded. Static. Size: 1.6 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/truchet-tiles.json
```
...
|---|---|---|---|
| `cell` | number | `32` | Width and height of each square tile, in pixels. |
| `tile` | "arcs" \| "diagonals" | `"arcs"` | Geometry drawn in each tile. |
| `thickness` | number | `1.5` | Width of each stroke, in pixels. |
| `strength` | number | `0.4` | Opacity of the strokes. |
| `seed` | number | `1` | Integer seed used to choose every tile orientation. |
...
// Pica · Truchet Tiles · truchet-tiles
...
// registry/patterns/truchet-tiles/core.ts
...
// registry/patterns/truchet-tiles/index.tsx
...
<title>Truchet Tiles · Pica</title>
...
<body>
<div id="pica"><h2>Wandering paths</h2><p>Seeded arcs connect across a field of square tiles.</p></div>
<script>
"use strict";
var PicaTruchetTiles = (() => {
  ...
  var TRUCHET_SVG_NS = "http://www.w3.org/2000/svg";
  ...
  function truchetPath(width, height, props) {
    const cell = truchetClamp(props.cell, 16, 64);
    const half = cell / 2;
    const columns = Math.ceil(width / cell);
    const rows = Math.ceil(height / cell);
  ...
  function truchetClamp(value, minimum, maximum) {
    return Math.max(minimum, Math.min(maximum, value));
  }
...
(function () {
  ...
  var instance = PicaTruchetTiles.mount(host, take(window.PICA_PROPS || {}));
