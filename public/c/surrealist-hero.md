...
> A hero behind which a dithered sea stands at two heights, under a sun reflected in the wrong shape.

Category: sections. Tags: hero, landing, section, surreal, dither, sea, horizon. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 6.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/surrealist-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"The sea stands at two heights."` | The large line at the top of the section. Empty leaves it out. |
| `subhead` | string | `"The same water meets the sky at two levels, and the sun reflects in the wrong shape."` | The smaller line under the headline. Empty leaves it out. |
| `actions` | readonly SurrealistHeroAction[] | `[{"label":"See the components","href":"#components"},{"label":"How it is built","href":"#docs"}]` | Calls to action, drawn as links. The first draws solid in the accent, the rest draw outline. |
...
| `intensity` | number | `0.45` | How strongly the dithered sea shows, from 0 to 1. |
| `horizon` | number | `66` | Height of the lower waterline, in percent of the host's height. |
| `step` | number | `22` | Height of the wall of water, in percent of the host's height. The upper waterline sits this far above the lower. |
| `seam` | number | `58` | Where the wall of water stands, in percent of the host's width. |
| `scale` | number | `4` | CSS pixels each dithered cell covers, passed to the sea. |
| `speed` | number | `0.45` | How fast the sea drifts, passed through to its own speed. 0 holds it still. |
...
// registry/sections/surrealist-hero/index.tsx
