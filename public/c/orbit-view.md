...
Category: immersive. Tags: 3d, map, earth, horizon, space, canvas. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 7.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/orbit-view.json
```
...
|---|---|---|---|
| `label` | string | `"Earth's limb from orbit"` | Text alternative naming the view. Empty hides the host from assistive technology. |
| `altitude` | number | `420` | Spacecraft height above the surface in kilometres. Higher sees more of the disc and a flatter limb. |
| `tilt` | number | `35` | Camera tilt away from straight down, in degrees. 0 looks at the sub-point, 80 nearly at the horizon. |
| `heading` | number | `110` | Compass azimuth of the camera's lean and of the ground track, in degrees clockwise from north. |
| `lat` | number | `42` | Starting latitude of the point under the spacecraft, in degrees north. |
| `lon` | number | `12` | Starting longitude of the point under the spacecraft, in degrees east. |
| `speed` | number | `0.15` | Ground track speed. 0 holds the sub-point at lat and lon. |
| `pitch` | number | `4` | Spacing of the land dots in CSS pixels. |
| `graticule` | number | `15` | Graticule spacing in degrees. 0 draws no graticule. |
| `atmosphere` | number | `0.6` | Strength of the dithered atmosphere bands above the limb. 0 draws no atmosphere. |
| `telemetry` | boolean | `true` | Show the telemetry block. |
| `fps` | number | `24` | Frames per second ceiling. |
| `fontFamily` | string | `"\"JetBrains Mono\", \"IBM Plex Mono\", ui-monospace, \"SFMono-Regular\", Menlo, monospace"` | Font for the telemetry readout. |
