...
> A hero behind a drifting lens slab that bends the printed field beneath its curved edge, screened into tone steps.

Category: sections. Tags: hero, liquid glass, lens, refraction, dither, section, landing. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 7.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/liquid-glass-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Glass that bends, never blurs."` | The headline, set large in the page's own font after anything the page wraps. Empty hides it. |
| `subhead` | string | `"A thick lens drifts over a printed field and displaces what lies beneath its curved edge, drawn in screened ink."` | Supporting copy under the headline, in the muted color. Empty hides it. |
| `actions` | readonly LiquidGlassHeroAction[] | `[{"label":"Get started","href":"#start"},{"label":"How it bends","href":"#how"}]` | Calls to action, drawn as links in source order. At most three: the first draws solid, the rest outline. |
...
| `size` | number | `0.34` | The slab's diameter as a share of the host's shorter side, 0.2 to 0.8. |
| `bend` | number | `0.6` | How far the slab's curved edge bends what lies behind it, from 0 (a flat sheet) to 1 (a strong lens). |
| `spacing` | number | `30` | Spacing of the printed lattice the slab drifts over, in CSS pixels. |
| `intensity` | number | `0.6` | How strongly the whole drawing inks, from 0 (the ground alone) to 1. |
| `drift` | number | `0.5` | How fast the slab drifts across the field. 0 holds it still. |
| `levels` | number | `4` | Tone steps the field is screened into: 2 is one-bit, 6 reads as nearly smooth. |
| `mask` | "blue" \| "bayer" | `"blue"` | The screen the field is dithered through: "blue" scatters the ink, "bayer" orders it. |
| `pixel` | number | `3` | CSS pixels each screened cell covers before the canvas is scaled up. |
