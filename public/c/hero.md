...
|---|---|---|---|
| `background` | "mesh" \| "noise" \| "none" | `"mesh"` | What draws behind the content: a mesh gradient, a drifting ASCII noise field, or nothing. |
| `actions` | readonly HeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links. The first draws solid in the accent, the rest draw outline. |
...
| `intensity` | number | `0.6` | How strongly the background shows, from 0 to 1, passed through to whichever one is mounted. |
...
<body>
<div id="pica"><h1>Components drawn in text.</h1><p>ASCII-first components for React and plain HTML.</p></div>
<script>
"use strict";
var PicaHero = (() => {
...
`;
  var FALLBACK = `radial-gradient(90% 70% at 30% 35%, color-mix(in srgb, ${cssVar("accent")} 70%, transparent), transparent 75%)`;
  function uniforms(p) {
    return { u_seed: p.seed, u_speed: p.speed, u_scale: p.scale, u_warp: p.warp, u_intensity: p.intensity, u_levels: p.levels };
  }
  ...
  function meshProps(p) {
    return { paused: p.paused, time: p.time, seed: p.seed, intensity: p.intensity };
  }
  function noiseProps(p) {
    return { paused: p.paused, time: p.time, seed: p.seed, contrast: noiseContrast(p.intensity) };
  }
  function mountBackground(kind, host, p) {
    const bgLayer = layer(host, "under");
    if (kind === "mesh") return { kind, layer: bgLayer, instance: mount2(bgLayer.el, meshProps(p)) };
    return { kind, layer: bgLayer, instance: mount(bgLayer.el, noiseProps(p)) };
  }
  function destroyBackground(bg) {
    if (bg.kind === "none") return;
    bg.instance.destroy();
    bg.layer.remove();
  }
  ...
  var mount3 = (host, initial = {}) => {
    ...
    actions.setAttribute("data-pica-actions", "");
    host.append(actions);
    let bg = props.background === "none" ? { kind: "none" } : mountBackground(props.background, host, props);
    sheet.setRules(rules(sheet.selector, props));
    renderActions(actions, props.actions);
    host.dataset.picaReady = "true";
    return {
      update(next) {
        ...
        if (props.background !== before.background) {
          ...
          if (props.paused !== before.paused || props.time !== before.time || props.seed !== before.seed || props.intensity !== before.intensity) {
            bg.instance.update(meshProps(props));
          }
        } else if (bg.kind === "noise") {
          if (props.paused !== before.paused || props.time !== before.time || props.seed !== before.seed || props.intensity !== before.intensity) {
...
(function () {
  ...
  var instance = PicaHero.mount(host, take(window.PICA_PROPS || {}));
