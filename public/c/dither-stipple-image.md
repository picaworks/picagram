# Dither Stipple Image

> An image redrawn as round stippled dots, spaced by a blue noise mask so tone reads as density, not rows.

Category: dither. Tags: image, static, dither, canvas, stipple. Static. Size: 4.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-stipple-image.json
```
...
| `dotSize` | number | `2` | Diameter of one dot, in CSS pixels. Every dot is the same size. |
| `density` | number | `1.2` | Overall dot spacing. Above 1 packs dots closer together; below 1 spreads them further apart. |
| `gamma` | number | `1` | Gamma applied to tone before it meets the mask. Above 1 pulls dots toward the darkest areas; below 1 spreads them into midtones. |
| `maskSize` | 16 \| 32 \| 64 | `64` | Side of the blue noise tile the mask reads from. A larger tile repeats less often, at more one-time setup cost. |
...
// Pica · Dither Stipple Image · dither-stipple-image
...
// registry/dither/dither-stipple-image/core.ts
...
// registry/dither/dither-stipple-image/index.tsx
...
<title>Dither Stipple Image · Pica</title>
  ...
  var mount = (host, initial = {}) => {
    ...
    function draw() {
      ...
      if (ctx && source && !failed) {
        ...
        const gridCols = Math.max(1, Math.ceil(w / minDist)) + 2;
        const gridRows = Math.max(1, Math.ceil(h / minDist)) + 2;
        const spatial = new Array(gridCols * gridRows);
        const minDist2 = minDist * minDist;
        const dotsX = [];
        const dotsY = [];
        scan:
          for (let r = 0; r < n; r++) {
            ...
            if (!sites) continue;
            for (const site of sites) {
              if (dotsX.length >= 6e4) break scan;
              const fx = site % fineCols;
              const fy = (site - fx) / fineCols;
              const px = (fx + 0.5) * fineW;
              const py = (fy + 0.5) * fineH;
              ...
              for (let oy = -1; oy <= 1 && !close; oy++) {
                const row = (gy + oy) * gridCols;
                for (let ox = -1; ox <= 1 && !close; ox++) {
                  const cell2 = spatial[row + gx + ox];
                  if (!cell2) continue;
                  for (let k = 0; k < cell2.length; k += 2) {
                    const dx = (cell2[k] ?? 0) - px;
                    const dy = (cell2[k + 1] ?? 0) - py;
                    if (dx * dx + dy * dy < minDist2) {
                      close = true;
                      break;
                    }
...
(function () {
  ...
  var instance = PicaDitherStippleImage.mount(host, take(window.PICA_PROPS || {}));
