...
> Contours of a slowly drifting terrain, each line drawn light and thin where it faces the light and dark and thick where it faces away.

Category: shaders. Tags: contour, topography, terrain, relief, shader, webgl, dither, background. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 5.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/contour-flow.json
```
...
|---|---|---|---|
| `speed` | number | `0.12` | How fast the terrain drifts. 0 holds it still. |
| `scale` | number | `1.8` | Size of the terrain across the frame: lower values are broad hills, higher values are busier. |
| `bands` | number | `16` | How many contours the field is cut into, at a constant interval. |
| `light` | number | `315` | Direction the light comes from, in degrees: 0 is the top, 90 the right, 315 the upper left. |
| `relief` | number | `0.7` | How strongly the light changes a line's weight and tone, from 0 (every line alike) to 1. |
| `index` | number | `5` | Every this many contours is drawn as an index contour in the accent. 0 draws none. |
| `thickness` | number | `1` | Width of a contour line in shader pixels before the relief thins or thickens it. |
| `levels` | number | `4` | Tone steps the lines are dithered between: 2 is one-bit, 16 reads as nearly smooth. |
| `pixel` | number | `1` | Size of one dither cell, in CSS pixels. |
...
`;
  var FALLBACK = `repeating-linear-gradient(115deg, color-mix(in srgb, ${cssVar("fg")} 30%, transparent) 0px, color-mix(in srgb, ${cssVar("fg")} 30%, transparent) 1px, transparent 1px, transparent 15px)`;
  function uniforms(p) {
    return {
      u_seed: p.seed,
      u_speed: p.speed,
...
(function () {
  ...
  var instance = PicaContourFlow.mount(host, take(window.PICA_PROPS || {}));
