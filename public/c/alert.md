...
Category: ui. Tags: alert, status, live region, notification, ui. Static. Size: 1.6 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/alert.json
```
...
|---|---|---|---|
| `tag` | string | `"note"` | The short label shown above the alert text. |
| `title` | string | `"Build finished"` | The alert heading announced to assistive technology. |
| `message` | string | `"All 24 components verified on both grounds."` | The supporting status message announced to assistive technology. |
| `tone` | "neutral" \| "muted" \| "accent" \| "filled" | `"accent"` | The treatment of the tag or the complete callout. |
...
var PicaAlert = (() => {
  ...
  function alertRules(selector, tone) {
    const fg = cssVar("fg");
    const accent = cssVar("accent");
    const muted = cssVar("muted");
    const onAccent = cssOn("accent");
  ...
  function alertPart(name, tagName = "div") {
    const element = document.createElement(tagName);
    element.setAttribute("data-pica", "");
    element.setAttribute("data-pica-part", name);
    return element;
...
(function () {
  ...
  var instance = PicaAlert.mount(host, take(window.PICA_PROPS || {}));
