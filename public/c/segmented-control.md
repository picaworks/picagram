...
Category: ui. Tags: segmented control, radio group, choice, form, ui. Static. Size: 2.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/segmented-control.json
```
...
|---|---|---|---|
| `options` | readonly SegmentedControlOption[] | `[{"value":"grid","label":"Grid","disabled":false},{"value":"list","label":"List","disabled":false},{"value":"board","label":"Board","disabled":false}]` | The choices shown in the ruled strip, in order. |
| `value` | string \| null | `null` | The checked value. Null means uncontrolled, so the component tracks its own choice. |
| `defaultValue` | string | `"grid"` | The value checked at mount, read once, while value is null. |
| `label` | string | `"View"` | The radio group's accessible name. |
| `disabled` | boolean | `false` | Blocks input and dims every segment. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `string` | The value of the segment the user checked. |
...
// Pica · Segmented Control · segmented-control
...
// registry/ui/segmented-control/core.ts
...
// registry/ui/segmented-control/index.tsx
...
<title>Segmented Control · Pica</title>
  ...
  var mount = (host, initial = {}) => {
    ...
    function rebuildIfNeeded() {
      ...
      entries = props.options.map((option) => {
        ...
        button.textContent = option.label;
        button.addEventListener("click", () => choose(entry));
        const entry = { value: option.value, disabled: option.disabled, button };
        host.append(button);
        return entry;
      });
    }
...
(function () {
  ...
  var instance = PicaSegmentedControl.mount(host, take(window.PICA_PROPS || {}));
