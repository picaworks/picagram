...
> A hero worn like a handled page: a frame missing one side, rules that stop short, and grain that gathers at the edges.

Category: sections. Tags: hero, wabi-sabi, imperfection, grain, frame, wear, section. Static. Size: 6.1 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/wabi-sabi-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Left unfinished on purpose."` | The headline, set large in the page's own font. Empty hides it. |
| `subhead` | string | `"An off centre hero whose boundary never closes, whose rules stop short, and whose wear gathers where hands would go."` | Supporting copy under the headline, drawn in the muted color. Empty hides it. |
| `actions` | readonly WabiSabiHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links in source order. The first draws solid in the foreground color, the rest draw hairline. At most three are drawn. |
| `align` | "start" \| "end" | `"start"` | Which side the content leans toward: "start" opens the void on the right, "end" mirrors the whole composition. |
| `frame` | boolean | `true` | Draw the enclosing frame, one side of it missing and every drawn side stopping short of its corner. |
| `wear` | number | `0.8` | How much speckle and staining the ground carries, from none at 0 to full wear at 1. |
...
| `seed` | number | `1` | Seed for every irregularity, so the same seed always finds the same imperfections. |
...
<title>Wabi Sabi Hero · Pica</title>
...
<body>
<div id="pica"><p>What remains is what matters: a boundary that never closes, lines that end early, and wear where hands have been.</p></div>
<script>
"use strict";
var PicaWabiSabiHero = (() => {
  ...
  var defaults = {
    headline: "Left unfinished on purpose.",
    subhead: "An off centre hero whose boundary never closes, whose rules stop short, and whose wear gathers where hands would go.",
    actions: [
      { label: "Browse components", href: "#components" },
      { label: "Read the docs", href: "#docs" }
    ],
  ...
  var ROW_GAP = "clamp(1rem, 2.6vh, 1.8rem)";
  var PAD_TOP = "clamp(4.5rem, 13vh, 8.5rem)";
  var PAD_BOTTOM = "clamp(3rem, 9vh, 6rem)";
  var PAD_SIDE = "clamp(1.25rem, 5vw, 4.5rem)";
  var GRAIN_SCALE = 0.5;
  var GRAIN_MAX = 24e4;
  var DRIFT_CELL = 5;
  var LINE_REACH = 14;
  var MAX_BACKING = 42e5;
  ...
  function sideStrokes(a, b, rng) {
    ...
    const marks = [...cuts.flat(), ...thins.flat()].filter((t) => t > s0 + 0.5 && t < len - s1 - 0.5).sort((m, n) => m - n);
    const inside = (ranges, t) => ranges.some(([x, y]) => t >= x && t <= y);
    const out = [];
    const base = 0.58 + rng() * 0.22;
    let prev = s0;
    for (const t of [...marks, len - s1]) {
  ...
  var mount = (host, initial = {}) => {
    ...
    function draw() {
      ...
      if (props.frame) {
        const ins = [m * (0.036 + rng() * 0.02), m * (0.036 + rng() * 0.02), m * (0.036 + rng() * 0.02), m * (0.036 + rng() * 0.02)];
        const f = { l: ins[3] ?? 0, t: ins[0] ?? 0, r: W - (ins[1] ?? 0), b: H - (ins[2] ?? 0) };
        const corners = [
          [[f.l, f.t], [f.r, f.t]],
          [[f.r, f.t], [f.r, f.b]],
          [[f.l, f.b], [f.r, f.b]],
      ...
      if (amount > 0) {
        ...
        if (bctx) {
          ...
          const ab = actionsEl.offsetWidth > 0 ? { l: actionsEl.offsetLeft - 24, t: actionsEl.offsetTop - 24, r: actionsEl.offsetLeft + actionsEl.offsetWidth + 24, b: actionsEl.offsetTop + actionsEl.offsetHeight + 24 } : null;
          const img = bctx.createImageData(gw, gh);
          const data = img.data;
          for (let y = 0; y < gh; y++) {
            const cy = (y + 0.5) / scale;
...
(function () {
  ...
  var instance = PicaWabiSabiHero.mount(host, take(window.PICA_PROPS || {}));
