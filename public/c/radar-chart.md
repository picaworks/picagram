...
```bash
npx shadcn@latest add https://picagram.dev/r/radar-chart.json
```
...
|---|---|---|---|
| `data` | { axes: { label: string }[]; series: { name: string; values: number[] }[] } | `{"axes":[{"label":"Speed"},{"label":"Size"},{"label":"Setup"},{"label":"Accessibility"},{"label":"Docs"},{"label":"Themes"}],"series":[{"name":"React","values":[80,60,90,85,70,95]},{"name":"Vue","values":[70,85,75,80,95,65]}]}` | Chart data with axes and series. Each series has a name and values matching the axis count. |
| `label` | string | `"Two libraries compared"` | Name for the chart, read by assistive technology and used as the hidden data table's caption. |
| `max` | number | `100` | Maximum value on the scale. |
| `rings` | number | `3` | Number of concentric rings. |
| `fill` | number | `0.12` | Opacity of the first series' fill, from 0 to 0.4. |
| `look` | "svg" \| "glyph" | `"svg"` | "svg" draws hairline axes and rings. "glyph" draws the same in a monospace grid. |
...
// registry/data/radar-chart/index.tsx
...
(function () {
  ...
  var instance = PicaRadarChart.mount(host, take(window.PICA_PROPS || {}));
