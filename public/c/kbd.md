...
Category: ui. Tags: keyboard, shortcut, keys, ui. Static. Size: 1.2 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/kbd.json
```
...
|---|---|---|---|
| `keys` | readonly string[] | `["⌘","K"]` | The key names in the chord, shown in order. |
| `variant` | "caps" \| "brackets" | `"caps"` | The look: "caps" draws physical key caps and "brackets" sets one bracketed monospace run. |
...
var PicaKbd = (() => {
  ...
  function kbdNode(tag, part, text) {
    const node = document.createElement(tag);
    node.setAttribute("data-pica", "");
    node.setAttribute("data-part", part);
    node.textContent = text;
...
(function () {
  ...
  var instance = PicaKbd.mount(host, take(window.PICA_PROPS || {}));
