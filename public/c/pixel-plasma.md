...
> The demoscene plasma as a printed screen: summed sinusoids drawn as coarse square pixels and dithered between a few tone steps on the GPU.

Category: shaders. Tags: plasma, shader, webgl, background, dither, demoscene. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/pixel-plasma.json
```
...
|---|---|---|---|
| `speed` | number | `0.2` | How fast the field drifts. 0 holds it still. |
| `scale` | number | `1.4` | Field size across the frame: lower values are broad lobes, higher values are busier. |
| `waves` | number | `4` | Sinusoids summed into the field. The last one rings out from a slowly moving centre. |
| `levels` | number | `5` | Tone steps the field is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
| `pixel` | number | `6` | Size of one drawn pixel, in CSS pixels. Coarseness is the point. |
...
// Pica · Pixel Plasma · pixel-plasma
...
// registry/shaders/pixel-plasma/core.ts
...
// registry/shaders/pixel-plasma/index.tsx
...
`;
  var FALLBACK = `radial-gradient(80% 70% at 42% 40%, color-mix(in srgb, ${cssVar("accent")} 30%, transparent), transparent 72%)`;
  function uniforms(p) {
    return { u_seed: p.seed, u_speed: p.speed, u_scale: p.scale, u_waves: p.waves, u_levels: p.levels };
  }
...
(function () {
  ...
  var instance = PicaPixelPlasma.mount(host, take(window.PICA_PROPS || {}));
