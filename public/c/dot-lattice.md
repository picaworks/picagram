...
> A calm field of small ink points on a regular or staggered lattice behind page content.

Category: patterns. Tags: dots, lattice, grid, background, css. Static. Size: 1.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dot-lattice.json
```
...
|---|---|---|---|
| `spacing` | number | `24` | Distance between neighboring lattice points, in pixels. |
| `angle` | number | `0` | Rotation of the lattice, in degrees. |
| `offset` | boolean | `false` | Staggers alternate rows by half of the spacing. |
| `dot` | number | `1` | Radius of each lattice point, in pixels. |
| `fade` | number | `0` | Portion of the field that fades toward the edges. |
| `strength` | number | `0.35` | Opacity of the lattice layer. |
...
// registry/patterns/dot-lattice/core.ts
...
// registry/patterns/dot-lattice/index.tsx
...
<body>
<div id="pica"><h2>Measured points</h2><p>A quiet lattice keeps the page in order.</p></div>
<script>
"use strict";
var PicaDotLattice = (() => {
  ...
  var SIZE_VAR = "--pica-dot-lattice-size";
  ...
  function rules(selector, p) {
    const point = `radial-gradient(circle at center,${cssVar("fg")} 0 ${p.dot}px,transparent ${p.dot}px)`;
    const backgrounds = p.offset ? `${point},${point}` : point;
    const sizes = p.offset ? `${p.spacing}px ${p.spacing * 2}px,${p.spacing}px ${p.spacing * 2}px` : `${p.spacing}px ${p.spacing}px`;
    const positions = p.offset ? `0 0,${p.spacing / 2}px ${p.spacing}px` : "0 0";
    const declarations = [
      "inset:50% auto auto 50%",
      `width:var(${SIZE_VAR})`,
...
(function () {
  ...
  var instance = PicaDotLattice.mount(host, take(window.PICA_PROPS || {}));
