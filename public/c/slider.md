...
Category: ui. Tags: slider, range, value, form, ui. Static. Size: 2.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/slider.json
```
...
|---|---|---|---|
| `value` | number \| null | `null` | The current value, or null to let the slider manage its own value. |
| `defaultValue` | number | `30` | The initial value when value is null. |
...
| `orientation` | "horizontal" \| "vertical" | `"horizontal"` | The direction in which the track runs. |
| `label` | string | `"Volume"` | The visible and accessible name of the slider. |
| `showValue` | boolean | `true` | Shows the current value beside the label. |
| `height` | number | `10` | The track height in rem when orientation is vertical. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `number` | The user chose a value with the keyboard or pointer. |
...
var PicaSlider = (() => {
  ...
  function sliderLimits(props) {
    return props.max >= props.min ? [props.min, props.max] : [props.min, props.min];
  }
  function sliderStep(props) {
    return Number.isFinite(props.step) && props.step > 0 ? props.step : 1;
  }
  function sliderSnap(raw, props) {
    const [min, max] = sliderLimits(props);
    const step = sliderStep(props);
    const clamped = Math.min(max, Math.max(min, raw));
    const snapped = min + Math.round((clamped - min) / step) * step;
  ...
  var mount = (host, initial = {}) => {
    ...
    function chooseFromPointer(event) {
      const rect = rail.getBoundingClientRect();
      const [min, max] = sliderLimits(props);
      const ratio = props.orientation === "vertical" ? (rect.bottom - event.clientY) / Math.max(1, rect.height) : (event.clientX - rect.left) / Math.max(1, rect.width);
      choose(min + Math.min(1, Math.max(0, ratio)) * (max - min));
    }
    ...
    return {
      update(next) {
        props = { ...props, ...next };
        if (next.value !== void 0 && next.value !== null) current = sliderSnap(next.value, props);
        else current = sliderSnap(current, props);
        render();
      },
      destroy() {
...
(function () {
  ...
  var instance = PicaSlider.mount(host, take(window.PICA_PROPS || {}));
