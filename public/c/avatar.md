...
> A square hairline avatar that shows a photo when available and centered mono initials while it loads or fails.

Category: ui. Tags: avatar, profile, image, initials, ui. Static. Size: 3.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/avatar.json
```
...
|---|---|---|---|
| `src` | string | `""` | The image URL or data URI. An empty value keeps the initials visible. |
| `name` | string | `"Ada Lin"` | The accessible name and the source of the first two initials. |
| `size` | number | `48` | The square frame size in pixels, from 24 through 128. |
| `dither` | boolean | `true` | Screens a loaded image into foreground ink and the exposed ground. |
...
var PicaAvatar = (() => {
  ...
  function avatarRules(selector, size) {
    const edge = avatarSize(size);
    return [
      `${selector}{display:inline-block;position:relative;vertical-align:middle;box-sizing:border-box;overflow:hidden;border:1px solid ${cssVar("fg")};border-radius:0;background:transparent;color:${cssVar("fg")}}`,
      `${selector}>[data-pica-avatar-initials]{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:${GRID_FONT};font-size:${Math.max(10, edge * 0.34)}px;font-weight:500;line-height:1;letter-spacing:0.04em;text-align:center;white-space:nowrap;pointer-events:none}`
...
(function () {
  ...
  var instance = PicaAvatar.mount(host, take(window.PICA_PROPS || {}));
