...
> Ghost blocks breathe between two quiet shade glyphs while content is loading.

Category: ui. Tags: skeleton, loading, placeholder, shimmer, ui. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/skeleton.json
```
...
|---|---|---|---|
| `avatar` | boolean | `true` | Draws a square placeholder before the text bands. |
| `heading` | boolean | `true` | Draws a wider heading band above the text lines. |
| `lines` | number | `3` | Number of text lines, from 0 to 8. The final line runs short. |
| `speed` | number | `0.5` | Pace of the grain breathing, from 0.25 to 2. |
| `label` | string | `"Loading"` | Accessible name for the loading state. An empty label hides the placeholder from assistive technology. |
| `fps` | number | `8` | Frames per second ceiling for the animation. |
...
Draws with `--pica-muted`. Set it on any ancestor, pass `palette` to the React component, or put `palette` in `window.PICA_PROPS` for the HTML file.
...
// registry/ui/skeleton/index.tsx
...
var PicaSkeleton = (() => {
  ...
  var CYCLE_MS = 4e3;
  var STILL_TIME = 1200;
  var LIGHT_SHADE = SHADES.charAt(1);
  var MEDIUM_SHADE = SHADES.charAt(2);
  var TAU = Math.PI * 2;
  function lineCount(lines) {
    return Math.max(0, Math.min(8, Math.round(lines)));
  }
  function layoutRows(p) {
    const count = lineCount(p.lines);
    const bodyStart = p.heading ? BAND_STEP : 0;
    const bodyEnd = count > 0 ? bodyStart + (count - 1) * BAND_STEP + 1 : 0;
    return Math.max(1, p.avatar ? AVATAR_ROWS : 0, p.heading ? 1 : 0, bodyEnd);
  ...
  var mount = (host, initial = {}) => {
    ...
    const grid = createGrid(host, gridOptions(host), onLayout);
    function glyphAt(x, y, t, reduced) {
      const seed = Math.round(props.seed);
      const sx = seed * 3;
      const sy = seed * 5;
      const threshold = bayerAt(8, x + sx, y + sy);
    ...
    function fillBand(x, y, width, height, t, reduced) {
      for (let row = 0; row < height; row++) {
        for (let col = 0; col < width; col++) grid.set(x + col, y + row, glyphAt(x + col, y + row, t, reduced));
      }
    }
