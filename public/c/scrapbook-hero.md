...
> A hero assembled like a scrapbook page, with seeded tilts, torn deckle edges, taped corners, and a pasted photocopy print around the page's own copy.

Category: sections. Tags: hero, scrapbook, paper, torn, tape, photocopy, landing, section. Static. Size: 8.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/scrapbook-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Pasted together on purpose."` | The headline, set largest on its own torn and taped slip. Empty hides the slip. |
| `subhead` | string | `"Torn edges, taped corners, and a photocopied study — every one of them drawn from a single seed."` | Supporting copy on a second slip, set smaller and offset from the headline. Empty hides it. |
| `actions` | readonly ScrapbookHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links. At most three: the first draws solid in the accent, the rest outline. |
| `align` | "start" \| "center" | `"start"` | Horizontal alignment of the slips within the host. |
| `caption` | string | `"fig. 01 · the built-in study, copied and pasted"` | The caption set under the pasted photocopy print, in small mono. Empty hides the caption line. |
| `note` | string | `"Field notes\nEvery tilt, tear and tape strip on this page comes from one seed."` | The note written on the ruled index-card scrap. Newlines start a new line. Empty hides the card. |
| `stub` | string | `"no. 09"` | The tiny label under the barcode stub tucked behind the photo. Empty hides the stub. |
| `footline` | string | `"picagram · sheet 09 of 12 · assembled by hand"` | The metadata line on the long slip pinned along the bottom edge. Empty hides the strip. |
| `seed` | number | `1` | Seed behind every rotation, tear, and tape placement, so the same seed lays out the same page. |
| `tilt` | number | `2` | The most a slip may tilt, in degrees. Each slip draws its own angle inside the range. |
| `tear` | number | `8` | The deepest a torn edge may bite into a slip, in pixels. Zero cuts every slip clean. |
...
// Pica · Scrapbook Hero · scrapbook-hero
...
const photocopyImage = (() => {
interface PhotocopyImageProps {
...
const defaults: PhotocopyImageProps = {
  src: "",
  alt: "",
  fit: "cover",
  tone: "auto",
...
const mount: Mount<PhotocopyImageProps> = (host, initial = {}) => {
...
// registry/sections/scrapbook-hero/core.ts
...
// registry/sections/scrapbook-hero/index.tsx
...
<title>Scrapbook Hero · Pica</title>
...
<body>
<div id="pica"><p>The page's own copy drops into the same layout, a little off the axis like everything else.</p></div>
<script>
"use strict";
var PicaScrapbookHero = (() => {
  ...
  var EDGE_STRONG = 0.5;
  var STREAK_GRID = 6;
  var STREAK_CHANCE = 0.05;
  var STREAK_MIN = 3;
  var STREAK_MAX = 9;
  var SPECKLE_CEILING = 15e-4;
  var SPECKLE_STREAM = 1;
  var STREAK_STREAM = 2;
  function photocopyBits(ink, w, h, p) {
    const radius = Math.max(1, Math.round(p.window / 2));
    const mean = boxMean(ink, w, h, radius);
    const bits = new Uint8Array(w * h);
    for (let i = 0; i < bits.length; i++) {
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          load();
        } else if (recolored || props.fit !== before.fit || props.tone !== before.tone || props.contrast !== before.contrast || props.window !== before.window || props.bias !== before.bias || props.speckle !== before.speckle || props.streaks !== before.streaks || props.invert !== before.invert || props.seed !== before.seed) {
          prepare();
        } else {
          draw();
        }
  ...
  function tapeClip(rng) {
    const j = () => `${((rng() * 2 - 1) * 1.3).toFixed(1)}px`;
    return `polygon(${j()} ${j()}, calc(100% + ${j()}) ${j()}, calc(100% + ${j()}) calc(100% + ${j()}), ${j()} calc(100% + ${j()}))`;
  }
  function tapeCss(rng, taken) {
    let c = Math.floor(rng() * 4);
    while (taken.includes(c)) c = (c + 1) % 4;
    const angle2 = (c === 0 || c === 2 ? -45 : 45) + (rng() * 2 - 1) * 10;
    const along = (-TAPE_LEN / 2 + (rng() * 2 - 1) * 0.3).toFixed(2);
    const off = (-TAPE_WID / 2 + (rng() * 2 - 1) * 0.12).toFixed(2);
    const side = c <= 1 ? `top:${off}em` : `bottom:${off}em`;
    const edge = c === 0 || c === 3 ? `left:${along}em` : `right:${along}em`;
    return { index: c, css: `${side};${edge};transform:rotate(${angle2.toFixed(1)}deg)` };
  }
  function indent(rng, center, base) {
    const rem = center ? (rng() * 2 - 1) * 1.1 : Math.max(0, base + (rng() * 2 - 1) * 0.7);
    return `${rem.toFixed(2)}rem`;
  }
  function slipRules(s, name, rng, p, style) {
    const fg = cssVar("fg");
    const center = p.align === "center";
    const rim = (16 + rng() * 6).toFixed(0);
    const paper = (9 + rng() * 4).toFixed(0);
    ...
    for (let i = 0; i < style.tapes; i++) {
      const tape = tapeCss(rng, taken);
      taken.push(tape.index);
      rules2.push(`${s} [data-pica-slip="${name}"] [data-pica-tape="${i}"]{${tape.css};clip-path:${tapeClip(rng)}}`);
    }
    return rules2;
  }
  function rules(s, p) {
  ...
  function part(tag, name, value) {
    const node = document.createElement(tag);
    node.setAttribute("data-pica", "");
    node.setAttribute(name, value);
    return node;
  }
  function slip(name, tapes) {
    const wrap = part("div", "data-pica-slip", name);
    const deckle = part("div", "data-pica-deckle", "");
    deckle.setAttribute("aria-hidden", "true");
    const paper = part("div", "data-pica-paper", "");
  ...
  function renderSlip(el, wrap, text) {
    el.textContent = text;
    wrap.hidden = text.trim() === "";
  }
  var mount2 = (host, initial = {}) => {
    ...
    host.prepend(head.wrap, sub.wrap);
    host.append(actionsEl, collage, metaSlip.wrap);
    const photoInstance = mount(print, { seed: props.seed, contrast: 1.15 });
