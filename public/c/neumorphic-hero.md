...
> A soft UI hero that trades blur for screens: a slab and a rail of stat tiles rise from the ground on dithered bevels, and the call to action presses in.

Category: sections. Tags: hero, neumorphic, soft ui, bevel, dither, section, landing. Static. Size: 5.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/neumorphic-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Raised out of the same ground."` | The headline, set large on the raised slab. Empty hides it. |
| `subhead` | string | `"A soft UI hero whose relief is screened, not blurred: each edge is two dithered tone steps, and every face keeps the ground color."` | Supporting copy under the headline, in the muted token. Empty hides it. |
| `actions` | readonly NeumorphicHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links in source order. The first presses into the slab, the rest rise from it. At most three show. |
| `facts` | readonly NeumorphicHeroFact[] | `[{"label":"Tone steps","value":"2"},{"label":"Screen","value":"32 × 32"},{"label":"Blur","value":"0 px"}]` | Facts for the stat tiles standing beside the slab, each a mono label and a figure in source order. At most three show; empty draws no rail. |
...
| `depth` | number | `0.8` | How much ink the screened tone steps lay down, from 0 (a bare hairline) to 1. |
| `seed` | number | `1` | Shifts the dither screen's placement, so two instances need not share a grain. |
...
<title>Neumorphic Hero · Pica</title>
...
var PicaNeumorphicHero = (() => {
  ...
  var defaults = {
    headline: "Raised out of the same ground.",
    subhead: "A soft UI hero whose relief is screened, not blurred: each edge is two dithered tone steps, and every face keeps the ground color.",
    actions: [
      { label: "Browse components", href: "#components" },
      { label: "Read the docs", href: "#docs" }
    ],
  ...
  var BEVEL_NEAR = 4;
  var BEVEL_FAR = 6;
  var BEVEL_REACH = 1 + BEVEL_NEAR + BEVEL_FAR;
  var NEAR_INK = 0.62;
  var FAR_INK = 0.3;
  var MAX_ACTIONS = 3;
  var MAX_FACTS = 3;
  ...
  var mount = (host, initial = {}) => {
    ...
    under.el.after(headlineEl, subheadEl, actionsEl);
    host.append(railEl);
    const parts = [headlineEl, subheadEl, actionsEl];
    ...
    function applyLayout() {
      ...
      for (const el of Array.from(host.children)) {
        if (el.hasAttribute("data-pica")) {
          if (parts.includes(el) && !el.hidden) rows++;
        } else rows++;
      }
      railEl.style.gridRow = min ? "" : `1 / span ${Math.max(1, rows)}`;
    }
    ...
    function draw() {
      ...
      const dark = fgLum > bgLum;
      const lit = dark ? fg : [255, 255, 255, 255];
      const shade = dark ? [0, 0, 0, 255] : fg;
      const mask = blueNoiseMatrix(SCREEN);
      const ox = hashSeed(props.seed, 3) % SCREEN;
      const oy = hashSeed(props.seed, 5) % SCREEN;
      const depth = Math.min(1, Math.max(0, props.depth));
      const near = NEAR_INK * depth;
      const far = FAR_INK * depth;
      function ring(xa, ya, xb, yb, ink, dist) {
        const xA = Math.max(0, Math.round(xa));
        const yA = Math.max(0, Math.round(ya));
        const xB = Math.min(w, Math.round(xb));
        const yB = Math.min(h, Math.round(yb));
      ...
      function bevel(x0, y0, x1, y1, tl, br, inset) {
        if (inset) {
          ring(x0, y0, x1, y0 + BEVEL_REACH, tl, (_x, y) => y - y0 + 1);
          ring(x0, y1 - BEVEL_REACH, x1, y1, br, (_x, y) => y1 - y);
          ring(x0, y0 + BEVEL_REACH, x0 + BEVEL_REACH, y1 - BEVEL_REACH, tl, (x) => x - x0 + 1);
      ...
      const raised = (el) => {
        const box2 = host.getBoundingClientRect();
        const r = el.getBoundingClientRect();
        if (r.width < 1 || r.height < 1) return;
        const x0 = Math.round(r.left - box2.left);
        const y0 = Math.round(r.top - box2.top);
        const x1 = Math.round(r.right - box2.left);
        const y1 = Math.round(r.bottom - box2.top);
        face(x0, y0, x1, y1);
        bevel(x0, y0, x1, y1, lit, shade, false);
      };
      const slab = contentRect();
...
(function () {
  ...
  var instance = PicaNeumorphicHero.mount(host, take(window.PICA_PROPS || {}));
