...
Category: effects. Tags: image, static, risograph, halftone, canvas. Static. Size: 4.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/risograph-image.json
```
...
| `offset` | number | `2` | Pixels the accent pass shifts from the fg pass, the misregistration a second drum leaves on the sheet. |
| `angle` | number | `200` | Degrees clockwise from three o'clock, the direction the accent pass shifts away from the fg pass. |
| `grain` | number | `4` | Pixels per screen cell, at the plate's own resolution. Coarser reads as a bigger dot at arm's length. |
| `split` | number | `0.45` | The tone that divides the two passes. Darker than this prints in fg; lighter prints in accent. |
| `coverage` | number | `0.85` | Ink a pass lays down at its fullest tone, short of solid so the paper always shows through. |
| `seed` | number | `1` | Seed for the screen's placement. The same seed always draws the same grain. |
...
// Pica · Risograph Image · risograph-image
...
// registry/effects/risograph-image/core.ts
...
// registry/effects/risograph-image/index.tsx
...
<title>Risograph Image · Pica</title>
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          load();
        } else if (recolored || props.fit !== before.fit || props.tone !== before.tone || props.contrast !== before.contrast || props.split !== before.split || props.coverage !== before.coverage || props.grain !== before.grain || props.seed !== before.seed) {
          prepare();
        } else {
          draw();
        }
...
(function () {
  ...
  var instance = PicaRisographImage.mount(host, take(window.PICA_PROPS || {}));
