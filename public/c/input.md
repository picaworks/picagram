...
> A native text field with controlled and uncontrolled values, accessible labeling, and keyboard event reporting.

Category: ui. Tags: input, text field, form, ui. Static. Size: 1.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/input.json
```
...
|---|---|---|---|
| `value` | string \| null | `null` | The displayed value. Null leaves the field uncontrolled. |
| `defaultValue` | string | `""` | The initial value when the field is uncontrolled. It is read only when mounted. |
| `type` | "text" \| "email" \| "password" \| "search" \| "url" | `"text"` | The kind of text the field accepts. |
| `placeholder` | string | `"Search components"` | The hint shown while the field is empty. |
| `label` | string | `"Search"` | The accessible name announced for the field. |
| `disabled` | boolean | `false` | Blocks editing and dims the field. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `string` | The complete field value after a user edit. |
| `enter` | `onEnter` | `string` | The complete field value when the user presses Enter. |
...
(function () {
  ...
  var instance = PicaInput.mount(host, take(window.PICA_PROPS || {}));
  ["valueChange","enter"].forEach(function (name) {
