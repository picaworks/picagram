# Dither Crosshatch Image

> An image drawn in crossing pen strokes, each layer switching on as the tone beneath it darkens.

Category: dither. Tags: image, static, crosshatch, canvas. Static. Size: 3.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-crosshatch-image.json
```
...
| `tone` | "auto" \| "light-on-dark" \| "dark-on-light" | `"auto"` | "auto" reads the host's colors. "light-on-dark" hatches the bright pixels; "dark-on-light" hatches the dark ones. |
| `contrast` | number | `1.1` | Contrast around mid grey. 1 leaves the image as it is. |
| `layers` | number | `3` | How many hatch layers can switch on, from a single wash to a dense mesh. |
| `spacing` | number | `6` | Distance between the strokes of one layer, in CSS pixels. |
| `angles` | readonly number[] | `[45,135,0,90]` | Angle of each layer's strokes, in degrees, one entry per layer. |
| `weight` | number | `1` | Stroke thickness in CSS pixels. |
| `jitter` | number | `0.3` | How far a stroke wanders from a straight line, 0 to 1. |
| `seed` | number | `1` | Seed for the jitter, so the same seed always draws the same picture. |
...
<title>Dither Crosshatch Image · Pica</title>
...
(function () {
  ...
  var instance = PicaDitherCrosshatchImage.mount(host, take(window.PICA_PROPS || {}));
