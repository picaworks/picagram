...
Category: ui. Tags: toast, notification, status, ui. Static. Size: 2.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/toast.json
```
...
|---|---|---|---|
| `toasts` | readonly ToastItem[] | `[]` | The notices to show, in stack order. |
| `duration` | number | `6000` | Milliseconds before a notice leaves, or zero to keep it until dismissal. |
| `position` | "top-start" \| "top-end" \| "bottom-start" \| "bottom-end" | `"bottom-end"` | The viewport corner that holds the stack. |
| `label` | string | `"Notifications"` | The accessible name for the live region. |
| `max` | number | `3` | The greatest number of notices visible at once. |
...
|---|---|---|---|
| `dismiss` | `onDismiss` | `string` | A notice left after its timer elapsed or its close button was pressed. |
...
var PicaToast = (() => {
  ...
  function toastRules(selector, position) {
    const fg = cssVar("fg");
    const muted = cssVar("muted");
    const accent = cssVar("accent");
    const blockSide = position.startsWith("top") ? "top:1rem!important;bottom:auto!important" : "top:auto!important;bottom:1rem!important";
    const inlineSide = position.endsWith("start") ? "left:1rem!important;right:auto!important" : "left:auto!important;right:1rem!important";
    return [
      `${selector}{position:fixed!important;${blockSide};${inlineSide};z-index:100;display:flex;flex-direction:column;gap:0.5rem;width:min(24rem,calc(100vw - 2rem))!important;height:auto!important;max-height:calc(100vh - 2rem);pointer-events:none}`,
      `${selector} [data-pica-toast]{display:grid;grid-template-columns:2px minmax(0,1fr) auto;align-items:stretch;border:1px solid ${muted};border-radius:0;background:color-mix(in srgb,${fg} 6%,transparent);color:${fg};pointer-events:auto}`,
      `${selector} [data-pica-rule]{background:${fg}}`,
  ...
  var mount = (host, initial = {}) => {
    ...
    function makeEntry(toast) {
      ...
      const entry = {
        ...
        title,
        message,
        remaining: Math.min(2e4, Math.max(0, props.duration)),
        started: 0,
        timer: null,
        hovered: false,
        focused: false
    ...
    function updateEntry(entry, toast) {
      entry.el.setAttribute("data-tone", toast.tone);
      entry.title.textContent = toast.title ?? "";
      entry.title.hidden = !toast.title;
      entry.message.textContent = toast.message;
      const close = entry.el.querySelector("[data-pica-close]");
      close?.setAttribute("aria-label", toast.title ? `Dismiss ${toast.title}` : "Dismiss notification");
    }
    function reconcile() {
      const supplied = new Set(props.toasts.map((toast) => toast.id));
    ...
    function apply(resetTimers) {
      attrs.set("aria-label", props.label);
      sheet.setRules(toastRules(sheet.selector, props.position));
      reconcile();
      if (resetTimers) {
        const duration = Math.min(2e4, Math.max(0, props.duration));
        for (const entry of entries.values()) {
          entry.remaining = duration;
          startTimer(entry);
        }
