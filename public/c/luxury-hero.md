...
> A hero built from space alone: a small tracked headline, one accent hairline, and text links with rules beneath them.

Category: sections. Tags: hero, landing, section, luxury, minimal, cta. Static. Size: 1.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/luxury-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Made To Last"` | The heading, set small in the page's own face with wide tracking. An empty string hides it. |
| `subhead` | string | `"Considered goods, produced in small runs and guaranteed for life."` | One or two lines of quiet copy under the headline, in the muted ink. An empty string hides it. |
| `actions` | readonly LuxuryHeroAction[] | `[{"label":"Explore the collection","href":"#collection"}]` | Calls to action, drawn as text links with a hairline under each. At most three are shown. |
| `align` | "center" \| "start" | `"center"` | Text alignment inside the centered column. |
| `minHeight` | number | `85` | The host's minimum height, in percent of the viewport height. |
...
// registry/sections/luxury-hero/core.ts
...
// registry/sections/luxury-hero/index.tsx
...
<body>
<div id="pica"><p>Every order is wrapped by hand and dispatched within two days.</p></div>
<script>
"use strict";
var PicaLuxuryHero = (() => {
...
(function () {
  ...
  var instance = PicaLuxuryHero.mount(host, take(window.PICA_PROPS || {}));
