...
> Rows showing measures as bars over muted bands that give qualitative ranges, with targets marked as ticks.

Category: data. Tags: chart, bullet, bars, bands, targets, svg, glyph grid, data table. Static. Size: 5.2 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/bullet-chart.json
```
...
|---|---|---|---|
| `data` | { label: string; value: number; target?: number; ranges: number[] }[] | `[{"label":"Revenue","value":270,"target":250,"ranges":[0,150,225,300]},{"label":"Signups","value":220,"target":200,"ranges":[0,100,200,300]},{"label":"Uptime","value":95,"ranges":[0,80,95,100]},{"label":"Latency","value":45,"target":40,"ranges":[0,50,100,150]}]` | Rows to plot, each with a label, value, optional target, and range bounds. Empty draws axes and a muted "no data" note. |
| `label` | string | `"Quarter against plan"` | Name for the chart, read by assistive technology and used as the hidden data table's caption. |
| `highlight` | number | `-1` | Index of the row drawn in the accent. -1 draws all rows in fg. |
| `ranges` | number | `3` | Number of range bands (muted at different opacities), 2 to 5. |
| `ticks` | number | `5` | About this many rounded ticks on the value axis, in the svg look. |
| `look` | "svg" \| "glyph" | `"svg"` | "svg" draws hairline ranges and rectangles. "glyph" draws the same bars in a monospace grid. |
...
// Pica · Bullet Chart · bullet-chart
...
// registry/data/bullet-chart/core.ts
...
// registry/data/bullet-chart/index.tsx
...
var PicaBulletChart = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    function renderTable() {
      ...
      const rows = props.data.map((d) => {
        const cells = [d.label, String(d.value)];
        if (d.target !== void 0) cells.push(String(d.target));
        return cells;
      });
      const headers = ["Label", "Value"];
      if (props.data.some((d) => d.target !== void 0)) headers.push("Target");
      table = dataTable(props.label || "Bullet chart", headers, rows);
      host.appendChild(table);
    }
    function drawSvg() {
      ...
      if (empty) {
        ...
        data.forEach((d, i) => {
          ...
          for (let r = 0; r < rangeCount; r++) {
            const rMin = d.ranges[r] ?? 0;
            const rMax = d.ranges[r + 1] ?? 0;
            const x12 = chartLeft + (rMin - first) / (last - first) * (chartRight - chartLeft);
            const x22 = chartLeft + (rMax - first) / (last - first) * (chartRight - chartLeft);
            const opacity = 0.2 + r / (rangeCount - 1 || 1) * 0.4;
            view.appendChild(
              svg("rect", {
                x: Math.max(chartLeft, Math.min(x12, x22)),
                y: rowY - bandHeight / 2,
                width: Math.abs(x22 - x12),
                height: bandHeight,
                fill: cssVar("muted"),
                opacity: String(opacity)
              })
...
(function () {
  ...
  var instance = PicaBulletChart.mount(host, take(window.PICA_PROPS || {}));
