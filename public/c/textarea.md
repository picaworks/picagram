...
Category: ui. Tags: textarea, form, input, ui. Static. Size: 2.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/textarea.json
```
...
|---|---|---|---|
| `value` | string \| null | `null` | The current text, or null to let the field manage its own value. |
| `defaultValue` | string | `"Shipped the palette work today.\nTwo lines in already."` | The text read once when an uncontrolled field mounts. |
| `label` | string | `"Notes"` | The visible name above the field. |
| `placeholder` | string | `"Write it down"` | The hint shown while the field is empty. |
| `maxRows` | number | `8` | The greatest number of visible lines before the field scrolls. |
| `max` | number | `280` | The suggested character limit shown below the field, or zero to hide the count. |
| `disabled` | boolean | `false` | Blocks editing and dims the complete control. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `string` | The field received text input. |
...
// registry/ui/textarea/index.tsx
...
var PicaTextarea = (() => {
  ...
  function textareaRules(selector) {
    const fg = cssVar("fg");
    const muted = cssVar("muted");
    const accent = cssVar("accent");
    return [
  ...
  function textareaLimit(value) {
    return Number.isFinite(value) ? Math.max(0, Math.floor(value)) : 0;
  }
  function sizeTextarea(field, maxRows) {
    field.style.height = "auto";
    const style = getComputedStyle(field);
    const fontSize = Number.parseFloat(style.fontSize) || 16;
    const lineHeight = Number.parseFloat(style.lineHeight) || fontSize * 1.5;
...
(function () {
  ...
  var instance = PicaTextarea.mount(host, take(window.PICA_PROPS || {}));
