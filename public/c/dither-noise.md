...
Category: dither. Tags: noise, fractal noise, halftone, background, cloud. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-noise.json
```
...
|---|---|---|---|
| `scale` | number | `2` | Noise cycles across the field's longer side. Higher values give smaller, more numerous clouds. |
| `octaves` | number | `3` | Fractal noise layers summed into the field, each finer than the last. |
| `levels` | number | `3` | Flat tone bands the field is screened into, from the ground up to full ink. |
| `mask` | "blue" \| "cluster" | `"blue"` | "blue" scatters ink for a photographic grain. "cluster" grows round dots for a newsprint screen. |
| `speed` | number | `0.15` | How fast the field drifts. 0 holds it still. |
| `angle` | number | `20` | Direction the field drifts toward, in degrees. 0 drifts rightward, 90 drifts downward. |
| `pixel` | number | `3` | CSS pixels each screened cell covers before the canvas is scaled up. |
| `contrast` | number | `1` | Contrast around mid grey, applied before the field is screened into bands. 1 leaves it as it is. |
...
// Pica · Dither Noise · dither-noise
...
// registry/dither/dither-noise/core.ts
...
// registry/dither/dither-noise/index.tsx
...
var PicaDitherNoise = (() => {
  ...
  function fieldAt(noise, nx, ny, scale, octaves) {
    const sx = nx * scale;
    const sy = ny * scale;
    const a = noiseSum(noise, sx, sy, octaves);
    const b = noiseSum(noise, sx - scale, sy, octaves);
  ...
  function tileDims(aspect) {
    const a = aspect > 0 ? aspect : 1;
    return a >= 1 ? [TILE_MAX, Math.max(8, Math.round(TILE_MAX / a))] : [Math.max(8, Math.round(TILE_MAX * a)), TILE_MAX];
  }
  function readField(tile, tileW, tileH, x, y) {
    const wx = (x % tileW + tileW) % tileW;
    const wy = (y % tileH + tileH) % tileH;
    const x0 = Math.floor(wx);
    const y0 = Math.floor(wy);
...
(function () {
  ...
  var instance = PicaDitherNoise.mount(host, take(window.PICA_PROPS || {}));
