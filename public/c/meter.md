...
> A labelled scalar readout drawn as block cells with an optional threshold marker.

Category: ui. Tags: meter, value, range, status, ui. Static. Size: 1.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/meter.json
```
...
|---|---|---|---|
| `value` | number | `64` | The current value shown by the meter. |
| `min` | number | `0` | The lower bound of the range. |
| `max` | number | `100` | The upper bound of the range. |
| `label` | string | `"Storage used"` | The visible and accessible name of the meter. An empty label hides it. |
| `marker` | number \| null | `80` | A threshold marked on the track, or null for no marker. |
| `valueText` | string | `""` | Text read by assistive technology instead of the raw value. |
| `width` | number | `24` | The width of the track in monospace cells. |
...
var PicaMeter = (() => {
  ...
  function meterClamp(value, low, high) {
    return Math.max(low, Math.min(high, value));
  }
  function meterRatio(value, min, max) {
    if (max === min) return value > min ? 1 : 0;
    return meterClamp((value - min) / (max - min), 0, 1);
  }
  function meterNode(tag, part) {
    const node = document.createElement(tag);
    node.setAttribute("data-pica", "");
    node.setAttribute("data-pica-part", part);
    return node;
...
(function () {
  ...
  var instance = PicaMeter.mount(host, take(window.PICA_PROPS || {}));
