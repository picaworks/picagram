...
> A semantic data table with tabular figures, quiet row rules, and optional three-state sorting.

Category: ui. Tags: table, data, sort, ui. Static. Size: 2.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/table.json
```
...
|---|---|---|---|
| `columns` | readonly TableColumn[] | `[{"key":"name","label":"Name"},{"key":"family","label":"Family"},{"key":"size","label":"Size KB","align":"right"},{"key":"checks","label":"Checks","align":"right"}]` | The keys, labels, and alignment that define the columns. |
| `rows` | readonly TableRow[] | `[{"name":"Quiet Hero","family":"Section","size":11.6,"checks":24},{"name":"Orbit Plot","family":"Data","size":6.4,"checks":22},{"name":"Glyph Atlas","family":"ASCII","size":5.8,"checks":18},{"name":"Signal Button","family":"UI","size":4.2,"checks":16},{"name":"Grid Field","family":"Pattern","size":3.7,"checks":12}]` | The records shown in the table body. |
| `label` | string | `"Components"` | The caption that names the table. |
| `sortable` | boolean | `true` | Lets each column heading cycle the table ordering. |
| `defaultSort` | TableSort \| null | `{"key":"size","direction":"desc"}` | The ordering read when the table mounts, or null to preserve row order. |
...
|---|---|---|---|
| `sortChange` | `onSortChange` | `{ key: string; direction: "asc" \| "desc" \| null }` | A heading changed the table ordering. |
...
var PicaTable = (() => {
  ...
  function tableElement(tag) {
    const node = document.createElement(tag);
    node.setAttribute("data-pica", "");
    return node;
  }
  function tableCompare(left, right) {
    if (typeof left === "number" && typeof right === "number") return left - right;
    const a = String(left).toLowerCase();
    const b = String(right).toLowerCase();
    return a < b ? -1 : a > b ? 1 : 0;
  }
  function tableRules(selector) {
    const fg = cssVar("fg");
    const muted = cssVar("muted");
    const accent = cssVar("accent");
    return [
  ...
  var mount = (host, initial = {}) => {
    ...
    let activeSort = props.defaultSort ? { ...props.defaultSort } : null;
...
(function () {
  ...
  var instance = PicaTable.mount(host, take(window.PICA_PROPS || {}));
  ["sortChange"].forEach(function (name) {
