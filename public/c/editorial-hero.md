...
Category: sections. Tags: editorial, hero, magazine, article, spread, drop cap, section. Static. Size: 6.1 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/editorial-hero.json
```
...
|---|---|---|---|
| `deck` | string | `"Notes on the craft"` | The deck, a small line under the top rule and above the headline. Empty hides it and its rule. |
| `headline` | string | `"The enduring craft of the printed page."` | The headline, set far larger than anything around it. Empty hides it. |
| `subhead` | string | `"What a magazine knows that a feed forgets: the eye wants a way in, a measure it can hold, and ink that trusts the paper it sits on, all of it decided before the first word is read."` | The standfirst, held to about sixty characters a line and opened by a drop capital. Empty hides it. |
| `byline` | string | `"By the Picagram desk"` | Who wrote the piece, set in mono at the left of the byline rules. Empty hides that half. |
| `date` | string | `"September 2026"` | When the piece ran, set in mono at the right of the byline rules. Empty hides that half. |
| `actions` | readonly EditorialHeroAction[] | `[{"label":"Read the story","href":"#story"},{"label":"Subscribe","href":"#subscribe"}]` | Calls to action, drawn as links. At most three show: the first solid in the accent, the rest outline. |
| `align` | "start" \| "center" | `"start"` | Horizontal alignment of the text column. |
| `src` | string | `""` | Image URL or data URI for the dithered plate beside the text. Empty shows no plate. |
| `alt` | string | `""` | Text alternative for the plate. Empty marks it decorative. |
...
// Pica · Editorial Hero · editorial-hero
...
const defaults: DitherImageProps = {
  src: "",
  alt: "",
  algorithm: "atkinson",
  scale: 3,
...
const mount: Mount<DitherImageProps> = (host, initial = {}) => {
...
// registry/sections/editorial-hero/core.ts
...
// registry/sections/editorial-hero/index.tsx
...
<title>Editorial Hero · Pica</title>
...
<body>
<div id="pica"><h2>The measure of a page</h2><p>Sixty characters is where a line stops being a guess and starts being a measure, and everything after the standfirst answers to it.</p></div>
<script>
"use strict";
var PicaEditorialHero = (() => {
  ...
  function threshold(values, width, height, level = 0.5, bayer = 0) {
    const out = new Uint8Array(width * height);
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const i = y * width + x;
  ...
  function diffuse(values, width, height, kernel) {
    const v = Float32Array.from(values);
    const out = new Uint8Array(width * height);
    const taps = KERNELS[kernel];
    for (let y = 0; y < height; y++) {
  ...
  function toBits(ink, cols, rows, algorithm) {
    if (algorithm === "floyd-steinberg" || algorithm === "atkinson") return diffuse(ink, cols, rows, algorithm);
    const size = algorithm === "bayer2" ? 2 : algorithm === "bayer4" ? 4 : 8;
    return threshold(ink, cols, rows, 0.5, size);
  }
  ...
  var defaults2 = {
    deck: "Notes on the craft",
    headline: "The enduring craft of the printed page.",
    subhead: "What a magazine knows that a feed forgets: the eye wants a way in, a measure it can hold, and ink that trusts the paper it sits on, all of it decided before the first word is read.",
    byline: "By the Picagram desk",
    date: "September 2026",
    actions: [
      { label: "Read the story", href: "#story" },
  ...
  var mount2 = (host, initial = {}) => {
    ...
    function mountPlate() {
      ...
      ready.observe(node, { attributes: true, attributeFilter: ["data-pica-ready"] });
      plate = { node, ready, instance: mount(node, { src: props.src, alt: props.alt }) };
    }
...
(function () {
  ...
  var instance = PicaEditorialHero.mount(host, take(window.PICA_PROPS || {}));
