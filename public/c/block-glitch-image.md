...
> A photograph that sits still, then briefly shows rectangular blocks copied in from the wrong place.

Category: effects. Tags: image, glitch, compression, blocks, canvas. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/block-glitch-image.json
```
...
| `block` | number | `24` | The fixed grid's cell size, in prepared-picture pixels. A displaced block is one cell, or smaller at an edge. |
| `blocks` | number | `12` | How many cells move in one burst. |
| `shift` | number | `0.12` | How far a moved cell's content can be sourced from, as a share of the picture's width. |
| `burst` | number | `220` | Milliseconds one burst lasts. |
| `rest` | number | `1400` | Milliseconds of quiet between one burst and the next. |
...
// Pica · Block Glitch Image · block-glitch-image
...
// registry/effects/block-glitch-image/core.ts
...
// registry/effects/block-glitch-image/index.tsx
...
<title>Block Glitch Image · Pica</title>
...
var PicaBlockGlitchImage = (() => {
  ...
  function buildBurst(index, cols, rows, plateWidth, props) {
    const rng = createRng(hashSeed(props.seed, index));
    const destRows = Math.max(1, Math.ceil(rows / 2));
    const destCells = cols * destRows;
    const count = Math.max(1, Math.min(props.blocks, destCells));
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          ...
          if (props.block !== before.block) updateGrid();
          else if (props.blocks !== before.blocks || props.shift !== before.shift || props.seed !== before.seed || props.burst !== before.burst || props.rest !== before.rest) {
            planIndex = -1;
          }
          loop.redraw();
        }
...
(function () {
  ...
  var instance = PicaBlockGlitchImage.mount(host, take(window.PICA_PROPS || {}));
