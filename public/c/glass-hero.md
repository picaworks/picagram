...
Category: sections. Tags: hero, glass, landing, section, dither, background. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 6.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/glass-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Glass, without the blur."` | The heading drawn after anything the page wraps. Empty draws none. |
| `subhead` | string | `"A frosted pane screened in dots, over a field that drifts."` | One line of support under the headline, drawn in the muted color. Empty draws none. |
| `actions` | readonly GlassHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links. At most three: the first draws solid in the accent, the rest outline. |
| `align` | "start" \| "center" | `"center"` | Horizontal alignment of the content block within the host. |
| `minHeight` | number | `68` | The host's minimum height, in percent of the viewport height. |
| `intensity` | number | `0.7` | How strongly the drifting field shows, from 0 to 1. |
| `frost` | number | `0.75` | How strongly the pane screens the field down, from 0 (the field passes through) to 1 (almost none gets through). |
| `scale` | number | `3` | CSS pixels each screened dot covers before the canvas is scaled up. |
| `speed` | number | `0.6` | How fast the field drifts. 0 holds it still. |
...
<body>
<div id="pica"><h1>A pane you can read through.</h1></div>
<script>
"use strict";
var PicaGlassHero = (() => {
...
(function () {
  ...
  var instance = PicaGlassHero.mount(host, take(window.PICA_PROPS || {}));
