...
Category: data. Tags: chart, scatter, svg, glyph grid, data table, regression. Static. Size: 6.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/scatter-plot.json
```
...
|---|---|---|---|
| `data` | { x: number; y: number; label?: string }[] | `[{"x":0,"y":10,"label":"Zero"},{"x":1,"y":12},{"x":2,"y":15},{"x":3,"y":18},{"x":4,"y":20,"label":"Four"},{"x":5,"y":22},{"x":6,"y":25},{"x":7,"y":28},{"x":8,"y":30},{"x":9,"y":32},{"x":1.5,"y":14},{"x":2.5,"y":16},{"x":3.5,"y":19},{"x":4.5,"y":21},{"x":5.5,"y":23},{"x":6.5,"y":26},{"x":7.5,"y":29},{"x":8.5,"y":31},{"x":0.5,"y":11},{"x":1.8,"y":13},{"x":3.2,"y":17},{"x":4.8,"y":21},{"x":6.2,"y":24},{"x":7.2,"y":27},{"x":8.2,"y":31},{"x":9.2,"y":33},{"x":2.2,"y":15},{"x":5.2,"y":23},{"x":6.8,"y":26},{"x":8.8,"y":32},{"x":0.8,"y":12},{"x":3.8,"y":19},{"x":7.8,"y":29},{"x":9.8,"y":33}]` | Points to plot, each with x, y, and optional label. Empty draws the axes and a muted "no data" note. |
| `label` | string | `"Score against hours studied"` | Name for the chart, read by assistive technology and used as the hidden data table's caption. |
| `highlight` | string[] | `["Zero","Four"]` | Labels of points to highlight and label directly. |
| `fit` | boolean | `false` | Draws a least-squares fit line through the data. |
| `xLabel` | string | `"Hours"` | Label for the x-axis. |
| `yLabel` | string | `"Score"` | Label for the y-axis. |
| `dotSize` | number | `2` | Radius of each point in CSS pixels. |
| `ticks` | number | `5` | About this many rounded ticks on each axis, in the svg look. |
| `look` | "svg" \| "glyph" | `"svg"` | "svg" draws hairline axes and circles. "glyph" draws the same points in a monospace grid. |
...
// registry/data/scatter-plot/index.tsx
