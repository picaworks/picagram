...
> A label, description, and validation message around a page-owned form control.

Category: ui. Tags: field, form, label, description, validation, error. Static. Size: 1.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/field.json
```
...
|---|---|---|---|
| `label` | string | `"Branch name"` | The visible name for the page-owned control. |
| `description` | string | `"Use a short name that describes the change."` | Supporting text shown below the control. |
| `error` | string | `""` | Validation text shown after the description. |
| `required` | boolean | `false` | Marks the label as required. |
| `controlId` | string | `"pica-field-control"` | The id of the page-owned control this field labels. |
...
(function () {
  ...
  var instance = PicaField.mount(host, take(window.PICA_PROPS || {}));
