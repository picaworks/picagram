...
> A hero that reads as a terminal held open too long, with scanlines over everything, a headline that tears in rare bursts, and readouts in the corners.

Category: sections. Tags: hero, cyberpunk, terminal, scanlines, glitch, section, cta, landing. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 6.2 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/cyberpunk-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Still transmitting."` | The headline, which the composed glitch text tears sideways in rare short bursts. Empty hides it. |
| `subhead` | string | `"The session has been open for 47 days. Every line still answers."` | The line under the headline, set in the page's own font. Empty hides it. |
| `kicker` | string | `"TERM 09 // UPLINK HELD"` | The mono status line above the headline, which a blinking block cursor ends. Empty hides it. |
| `actions` | readonly CyberpunkHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as links. At most three show, and the first fills with the foreground. |
| `align` | "start" \| "center" | `"start"` | Horizontal alignment of the content column within the host. |
| `minHeight` | number | `68` | The host's minimum height, in percent of the viewport height. |
| `telemetry` | boolean | `true` | Mono readouts stacked in the corners, ticking once a second with the session clock. |
| `scanlines` | boolean | `true` | The scanline overlay drawn over the whole section, with a slow roll band. |
| `intensity` | number | `0.55` | How strongly the scanlines show, from 0 to 1. |
...
<title>Cyberpunk Hero · Pica</title>
...
<body>
<div id="pica"><p>The page's own copy lands here, between the lead and the links.</p></div>
<script>
"use strict";
var PicaCyberpunkHero = (() => {
  ...
  function animatedText(host, text, tag = "span") {
    const hidden = hiddenText(text);
    hidden.setAttribute("data-pica", "");
    const layer2 = document.createElement(tag);
    layer2.setAttribute("data-pica", "");
    layer2.setAttribute("aria-hidden", "true");
    host.append(hidden, layer2);
    return {
      layer: layer2,
      setText(next) {
        hidden.textContent = next;
  ...
  var SLICE_COUNT = 4;
  var MAX_SHIFT_CH = 1.2;
  var MAX_SWAP_CHANCE = 0.35;
  function burstAt(t, interval, burst) {
    const period = Math.max(1, interval);
    const duration = Math.min(Math.max(0, burst), period);
    const phase = period * 0.4;
    const shifted = t - phase;
  ...
  function buildBurst(props, index) {
    const rng = createRng(hashSeed(props.seed, index));
    const ramp = measureRamp(props.glyphs, props.fontFamily);
    const swapChance = MAX_SWAP_CHANCE * props.intensity;
    const text = Array.from(props.text).map((ch) => {
      if (rng() >= swapChance) return ch;
      const at = Math.min(ramp.glyphs.length - 1, Math.floor(rng() * ramp.glyphs.length));
      return ramp.glyphs[at] ?? ch;
    }).join("");
  ...
  var ACCENT_CORNER = 1;
  var TEAR_INTERVAL = 4800;
  var TEAR_BURST = 240;
  var TEAR_FPS = 8;
  var ROLL_MS = 14e3;
  var CURSOR_MS = 530;
  var NARROW = "44rem";
  var INSET = "clamp(1.1rem, 3.5vw, 3rem)";
  var ROLL_BAND2 = `linear-gradient(to bottom, transparent 0%, transparent 38%, ${cssVar("fg")} 50%, transparent 62%, transparent 100%)`;
  function unit(value) {
    return Math.min(1, Math.max(0, value));
  }
  ...
  function scanOpacity(intensity) {
    return (0.05 + unit(intensity) * 0.3).toFixed(3);
  }
  function scanProps(p) {
    return { paused: p.paused, time: p.time, seed: p.seed, opacity: 1, roll: false, fps: 6 };
  }
  function tearProps(p) {
    return { text: p.headline, interval: TEAR_INTERVAL, burst: TEAR_BURST, intensity: 0.6, fontFamily: "inherit", fps: TEAR_FPS, paused: p.paused, time: p.time, seed: p.seed };
  }
  function readouts(seed, epoch) {
    const jitter = (lane) => createRng(hashSeed(seed, epoch, lane))();
    const base = createRng(hashSeed(seed, 0, 77));
    const session = Math.floor(base() * 65535);
    const up = Math.floor(base() * 4e6) + epoch;
    const pkt = 4e5 + Math.floor(base() * 8e5) + epoch * (120 + Math.floor(base() * 300));
    const dd = Math.floor(up / 86400);
    const hh = Math.floor(up / 3600) % 24;
    const mm = Math.floor(up / 60) % 60;
    const ss = up % 60;
  ...
  function rules2(s, p) {
    ...
    return [
      ...
      // a translateY in percent of its own box: half of it is exactly the host's height, which is the shift
      // the old background-position math needed, and a percentage needs no layout read on the frame.
      `${s} [data-pica-band]{position:absolute;top:-100%;left:0;right:0;height:200%;z-index:1;will-change:transform;background-image:${ROLL_BAND2};background-size:100% 50%;background-repeat:repeat-y}`,
      `@media (max-width: ${NARROW}){${s} [data-pica-corner][data-pica-pos="bl"],${s} [data-pica-corner][data-pica-pos="br"]{display:none}}`
    ].join("\n");
  }
  ...
  var mount3 = (host, initial = {}) => {
    ...
    function mountScan() {
      ...
      scanLayer.el.style.overflow = "hidden";
      scanLayer.el.style.opacity = scanOpacity(props.intensity);
      scan = mount2(scanLayer.el, scanProps(props));
      bandEl = part("span", "band");
      scanLayer.el.append(bandEl);
    }
    function dropScan() {
    ...
    if (props.scanlines) mountScan();
    const tear = mount(headlineEl, tearProps(props));
    let epoch = -1;
    ...
    const clock = createLoop({ el: host, fps: 30, paused: props.paused, time: props.time, still: 0, frame: draw });
    sheet.setRules(rules2(sheet.selector, props));
    kickerEl.hidden = props.kicker.trim() === "";
    subheadEl.textContent = props.subhead;
    subheadEl.hidden = props.subhead.trim() === "";
    headlineEl.hidden = props.headline.trim() === "";
...
(function () {
  ...
  var instance = PicaCyberpunkHero.mount(host, take(window.PICA_PROPS || {}));
