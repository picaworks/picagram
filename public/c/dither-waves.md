...
> Wave trains crossing at different angles, their interference screened by an ordered mask into a moire lattice.

Category: dither. Tags: moire, interference, ordered dither, background. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-waves.json
```
...
|---|---|---|---|
| `trains` | number | `3` | Number of overlapping wave trains, from two to four. |
| `period` | number | `34` | Wavelength of each wave train, in computed pixels. |
| `spread` | number | `27` | Degrees of angle from one wave train to the next. |
| `angle` | number | `0` | Angle of the first wave train, in degrees. |
| `levels` | number | `2` | Ink levels the interference is screened into, from two to four. |
| `mask` | "blue" \| "cluster" | `"cluster"` | Ordered mask that screens the interference: an even blue noise scatter, or a clustered dot screen. |
| `pixel` | number | `2` | CSS pixels each computed pixel covers. |
| `speed` | number | `0.12` | How fast the wave trains drift. 0 holds them still. |
...
// Pica · Dither Waves · dither-waves
...
// registry/dither/dither-waves/index.tsx
...
(function () {
  ...
  var instance = PicaDitherWaves.mount(host, take(window.PICA_PROPS || {}));
