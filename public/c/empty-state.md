...
Category: ui. Tags: empty, placeholder, message, action, ui. Static. Size: 1.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/empty-state.json
```
...
|---|---|---|---|
| `mark` | "tray" \| "magnifier" \| "orbit" | `"tray"` | The box drawn figure shown above the title. |
| `title` | string | `"Nothing here yet"` | The primary empty state message. |
| `message` | string | `"Components you add will show up in this list."` | The supporting text shown below the title. |
| `action` | EmptyStateAction \| null | `{"label":"Add a component","href":"#"}` | The optional link shown below the message. |
...
// registry/ui/empty-state/core.ts
...
// registry/ui/empty-state/index.tsx
...
var PicaEmptyState = (() => {
  ...
  function emptyStateRules(selector) {
    const fg = cssVar("fg");
    const accent = cssVar("accent");
    const muted = cssVar("muted");
    return [
  ...
  function emptyStateNode(tag, part) {
    const node = document.createElement(tag);
    node.setAttribute("data-pica", "");
    node.setAttribute("data-part", part);
    return node;
...
(function () {
  ...
  var instance = PicaEmptyState.mount(host, take(window.PICA_PROPS || {}));
