...
> A hero section that shows its structure: bordered blocks that collide, mono labels, and the accent as flat fill.

Category: sections. Tags: hero, brutalist, section, borders, cta, landing. Static. Size: 2.9 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/brutalist-hero.json
```
...
|---|---|---|---|
| `headline` | string | `"Structure, laid bare."` | The headline, set heavy and uppercase inside its own bordered block. Empty hides it. |
| `subhead` | string | `"Every box shows its edges. Borders run thick, corners stay square, and the accent fills a whole block."` | The line under the headline, in a second bordered block shifted off the headline's edge. Empty hides it. |
| `kicker` | string | `"Spec / 09"` | A short label on the accent block that overlaps the headline's top edge. Empty hides it. |
| `actions` | readonly BrutalistHeroAction[] | `[{"label":"Browse components","href":"#components"},{"label":"Read the docs","href":"#docs"}]` | Calls to action, drawn as bordered links. At most three show, and the first fills with the accent. |
| `background` | "grid" \| "rules" \| "none" | `"grid"` | What the underlay draws behind everything: a faint square grid, ruled lines, or nothing. |
| `intensity` | number | `0.6` | How strongly the underlay shows, from 0 to 1. |
| `align` | "start" \| "center" | `"start"` | Horizontal alignment of the content blocks within the host. |
...
// Pica · Brutalist Hero · brutalist-hero
...
// registry/sections/brutalist-hero/core.ts
...
// registry/sections/brutalist-hero/index.tsx
...
<title>Brutalist Hero · Pica</title>
...
<body>
<div id="pica"><h2>Every block shows its edges.</h2><p>Content the page wraps lands in its own bordered box, colliding with the rest.</p></div>
<script>
"use strict";
var PicaBrutalistHero = (() => {
  ...
  function rules(s, p) {
    ...
    const textAlign = p.align === "center" ? "center" : "start";
    const line = `color-mix(in srgb, ${fg} ${Math.round(5 + unit(p.intensity) * 11)}%, transparent)`;
    const underlay = p.background === "grid" ? `background-image:linear-gradient(to right, ${line} 1px, transparent 1px),linear-gradient(to bottom, ${line} 1px, transparent 1px);background-size:44px 44px` : p.background === "rules" ? `background-image:linear-gradient(to bottom, ${line} 1px, transparent 1px);background-size:44px 32px` : "background-image:none";
    return [
      `:where(${s}){min-height:${vh(p.minHeight)}vh}`,
      `${s}{box-sizing:border-box;position:relative;display:flex;flex-direction:column;justify-content:center;align-items:${edge};border:4px solid ${fg};padding:clamp(3rem,7vh,5rem) clamp(1.25rem,5vw,4rem) calc(clamp(1.25rem,5vw,4rem) + 4em);color:${fg};overflow-wrap:break-word}`,
      `${s} > [data-pica-under]{${underlay}}`,
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (!sameJson(before.actions, props.actions)) renderActions(actions, props.actions);
        if (props.align !== before.align || props.minHeight !== before.minHeight || props.background !== before.background || props.intensity !== before.intensity) {
          sheet.setRules(rules(sheet.selector, props));
        }
      },
      destroy() {
...
(function () {
  ...
  var instance = PicaBrutalistHero.mount(host, take(window.PICA_PROPS || {}));
