...
> A plane divided into territories around slowly drifting sites, each cell a flat dithered accent tone behind a fg hairline.

Category: shaders. Tags: voronoi, cells, shader, webgl, background, dither. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/voronoi-drift.json
```
...
|---|---|---|---|
| `speed` | number | `0.12` | How fast the sites drift. 0 holds them still. |
| `cells` | number | `14` | How finely the plane divides: a low value is a few large cells, a high value a dense mosaic. |
| `border` | number | `1` | Width of the hairline between cells, in CSS pixels. 0 draws no border. |
| `jitter` | number | `0.9` | How far a site wanders from its lattice point, from 0 (a fixed square grid) to 1 (nearly the whole cell). |
| `levels` | number | `4` | Tone steps a cell's fill is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
| `pixel` | number | `2` | Size of one drawn pixel, in CSS pixels. |
...
// Pica · Voronoi Drift · voronoi-drift
...
// registry/shaders/voronoi-drift/core.ts
...
// registry/shaders/voronoi-drift/index.tsx
...
<title>Voronoi Drift · Pica</title>
...
(function () {
  ...
  var instance = PicaVoronoiDrift.mount(host, take(window.PICA_PROPS || {}));
