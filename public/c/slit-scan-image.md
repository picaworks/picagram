...
Category: effects. Tags: image, slit-scan, streak, canvas. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/slit-scan-image.json
```
...
| `axis` | "horizontal" \| "vertical" | `"horizontal"` | The sweep's direction: across a horizontal frame, or down a vertical one. |
| `width` | number | `8` | The slit's width in pixels, and how far behind it a column still keeps its own place before it lags. |
| `amount` | number | `0.5` | How far the sampled column lags the drawn one once it falls behind the slit: 0 for none, 1 for the most. |
| `speed` | number | `0.25` | How fast the slit sweeps across the frame, 0 for nearly still and 1 for fastest. |
| `wrap` | boolean | `true` | Wraps the sweep back to the start instead of reversing direction. |
...
// Pica · Slit Scan Image · slit-scan-image
...
// registry/effects/slit-scan-image/core.ts
...
// registry/effects/slit-scan-image/index.tsx
...
<title>Slit Scan Image · Pica</title>
...
var PicaSlitScanImage = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    function slitAt(t, length) {
      const p = period();
      if (props.wrap) {
        const cycle2 = (t % p + p) % p;
        return cycle2 / p * length;
      }
      const span = p * 2;
      const cycle = (t % span + span) % span;
      const frac = cycle <= p ? cycle / p : 2 - cycle / p;
