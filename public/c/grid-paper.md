...
> A quiet square mesh with darker major rules aligned to regular minor intervals.

Category: patterns. Tags: grid, graph paper, engineering, background, css. Static. Size: 1.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/grid-paper.json
```
...
|---|---|---|---|
| `spacing` | number | `8` | Width and height of each minor cell, in pixels. |
| `major` | number | `5` | Number of minor cells between major rules. |
| `strength` | number | `0.4` | Opacity of the complete grid, from quiet to fully visible. |
...
// registry/patterns/grid-paper/core.ts
...
// registry/patterns/grid-paper/index.tsx
...
<body>
<div id="pica"><div><h2>Working Grid</h2><p>Measured notes for careful construction.</p></div></div>
<script>
"use strict";
var PicaGridPaper = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        const before = props;
        props = { ...props, ...next };
        if (props.spacing !== before.spacing || props.major !== before.major || props.strength !== before.strength) {
          sheet.setRules(gridRules(sheet.selector, props));
        }
      },
      destroy() {
  ...
  function gridRules(selector, props) {
    const spacing = Math.min(32, Math.max(4, props.spacing));
    const major = Math.min(10, Math.max(2, Math.round(props.major)));
    const period = spacing * major;
    const strength = Math.min(1, Math.max(0, props.strength));
...
(function () {
  ...
  var instance = PicaGridPaper.mount(host, take(window.PICA_PROPS || {}));
