...
Category: ui. Tags: switch, toggle, form, ui. Static. Size: 1.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/switch.json
```
...
|---|---|---|---|
| `checked` | boolean \| null | `null` | The current state. Null leaves the switch uncontrolled. |
| `defaultChecked` | boolean | `false` | The initial state when the switch is uncontrolled. |
| `label` | string | `"Grid lines"` | The visible name beside the switch. |
| `showState` | boolean | `true` | Shows a small On or Off value after the label. |
| `disabled` | boolean | `false` | Blocks input and dims the switch. |
...
|---|---|---|---|
| `checkedChange` | `onCheckedChange` | `boolean` | The user requested a new checked state. |
...
(function () {
  ...
  var instance = PicaSwitch.mount(host, take(window.PICA_PROPS || {}));
