...
> A slow flight down a tunnel drawn as a hairline grid, mapped by angle and inverse distance into a receding texture.

Category: shaders. Tags: tunnel, grid, shader, webgl, background, dither, perspective. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/tunnel-grid.json
```
...
|---|---|---|---|
| `speed` | number | `0.15` | How fast the flight runs, as rings per second of texture travel. 0 holds the tunnel still. |
| `rings` | number | `14` | Ring density along the tunnel: lower values space the depth out, higher values crowd it. |
| `spokes` | number | `24` | Radial lines running the length of the tunnel. |
| `fade` | number | `0.12` | Fraction of the radius around the vanishing point that stays empty, hiding where the mapping compresses past the pixel scale. |
| `twist` | number | `0.15` | How far the spokes spiral as they recede, from 0 (straight) to 1 (a deep wind). |
| `levels` | number | `4` | Tone steps the grid is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
| `pixel` | number | `1` | Size of one shader pixel, in CSS pixels. |
...
// registry/shaders/tunnel-grid/core.ts
...
// registry/shaders/tunnel-grid/index.tsx
...
`;
  var FALLBACK = `repeating-radial-gradient(circle at 50% 50%, color-mix(in srgb, ${cssVar("fg")} 55%, transparent) 0 1px, transparent 1px 42px)`;
  function uniforms(p) {
    return {
      u_seed: p.seed,
      u_speed: p.speed,
...
(function () {
  ...
  var instance = PicaTunnelGrid.mount(host, take(window.PICA_PROPS || {}));
