...
> A labeled band of action and toggle controls with roving keyboard focus.

Category: ui. Tags: toolbar, actions, toggle, keyboard, ui. Static. Size: 2.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/toolbar.json
```
...
|---|---|---|---|
| `items` | readonly ToolbarItem[] | `[{"id":"run","label":"Run"},{"id":"format","label":"Format"},{"id":"lint","label":"Lint"},{"id":"divide","label":"Tools","separator":true},{"id":"docs","label":"Docs","toggle":true,"pressed":true},{"id":"settings","label":"Settings"}]` | The controls and dividing rules in display order. |
| `label` | string | `"Editor"` | The accessible name for the toolbar. |
...
|---|---|---|---|
| `press` | `onPress` | `string` | An action control was activated. |
| `toggle` | `onToggle` | `{ id: string; pressed: boolean }` | A toggle control changed state. |
...
var PicaToolbar = (() => {
  ...
  function toolbarRules(selector) {
    const fg = cssVar("fg");
    const accent = cssVar("accent");
    return [
      `${selector}{display:inline-flex;align-items:stretch;width:max-content;max-width:100%;box-sizing:border-box;border:1px solid ${fg};border-radius:0;background:transparent;color:${fg};vertical-align:middle}`,
  ...
  var mount = (host, initial = {}) => {
    ...
    const nodes = [];
    const controls = [];
    const itemFor = /* @__PURE__ */ new Map();
    ...
    function renderItems() {
      const activeId = itemFor.get(controls[active])?.id;
      clearNodes();
      pressed.clear();
      for (const item of props.items) {
        if (item.separator) {
    ...
    const onClick = (event) => {
      const control = event.target.closest("[data-pica-control]");
      if (!control || !itemFor.has(control)) return;
      const item = itemFor.get(control);
      if (!item) return;
      active = controls.indexOf(control);
...
(function () {
  ...
  var instance = PicaToolbar.mount(host, take(window.PICA_PROPS || {}));
  ["press","toggle"].forEach(function (name) {
