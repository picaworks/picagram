...
> Slow blobs merge and part like drops of mercury, each surface mirroring a two band horizon through summed fields and reflection mapping.

Category: shaders. Tags: shader, webgl, background, dither, metaballs, reflection. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 5.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/liquid-metal.json
```
...
|---|---|---|---|
| `speed` | number | `0.12` | How fast the blobs drift. 0 holds them still. |
| `blobs` | number | `3` | How many centres contribute to the field. |
| `size` | number | `0.16` | Size of each drop, as a fraction of the host's short side. |
| `threshold` | number | `0.5` | Field sum that counts as surface: lower values fatten every blob and merge them sooner. |
| `horizon` | number | `0.55` | Where the horizon sits in the reflected environment, from 0 (all ground) to 1 (all sky). |
| `pointer` | boolean | `true` | Whether the nearest blob leans toward the pointer. |
| `levels` | number | `6` | Tone steps the metal is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
...
// Pica · Liquid Metal · liquid-metal
...
// registry/shaders/liquid-metal/core.ts
...
// registry/shaders/liquid-metal/index.tsx
...
`;
  var FALLBACK = `linear-gradient(to bottom, transparent 30%, color-mix(in srgb, ${cssVar("fg")} 20%, transparent) 46%, color-mix(in srgb, ${cssVar("accent")} 45%, transparent) 55%, color-mix(in srgb, ${cssVar("fg")} 14%, transparent) 63%, transparent 78%)`;
  function uniforms(p) {
    return {
      u_seed: p.seed,
      u_speed: p.speed,
...
(function () {
  ...
  var instance = PicaLiquidMetal.mount(host, take(window.PICA_PROPS || {}));
