...
Category: data. Tags: chart, timeline, spans, events, svg, glyph, data table. Static. Size: 5.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/timeline-chart.json
```
...
|---|---|---|---|
| `data` | TimelineRow[] | `[{"label":"Research","start":"2025-09-01","end":"2025-10-15"},{"label":"Runtime","start":"2025-10-01","end":"2025-12-31","events":[{"time":"2025-11-15","label":"v1"}]},{"label":"Wave 4","start":"2025-10-15","end":"2026-03-31","events":[{"time":"2025-12-15","label":"build"}]},{"label":"Review","start":"2026-03-15","end":"2026-06-30"},{"label":"Launch","start":"2026-06-01","events":[{"time":"2026-09-15","label":"ship"}]}]` | Rows of spans and events over time. Empty draws the axes and a muted "no data" note. |
| `label` | string | `"Project timeline"` | Name for the chart, read by assistive technology and used as the hidden data table's caption. |
| `highlight` | number | `-1` | Index of the span or event highlighted in the accent. -1 highlights nothing. |
| `look` | "svg" \| "glyph" | `"svg"` | "svg" draws hairline axes and rectangles. "glyph" draws in a monospace grid. |
| `ticks` | number | `6` | About this many rounded ticks on the time axis. |
| `rowHeight` | number | `24` | Height of each row in CSS pixels. |
...
<title>Timeline Chart · Pica</title>
...
var PicaTimelineChart = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    function drawSvg() {
      ...
      const data = props.data;
      const empty = data.length === 0;
      const [timeMin, timeMax] = empty ? [0, 864e5] : timeRange(data);
      const { times } = empty ? { times: [] } : timeTicks(timeMin, timeMax, props.ticks);
      const first = times[0] ?? timeMin;
      const last = times[times.length - 1] ?? timeMax;
      const cell = measureCell(props.fontFamily, fontSize, 1);
    ...
    function drawGlyph() {
      ...
      if (data.length === 0) {
        const note = "no data";
        const x2 = Math.max(0, Math.floor((cols - note.length) / 2));
        const y = Math.floor(rows / 2);
        g.write(x2, y, note, colors.muted);
        g.flush();
        host.dataset.picaReady = "true";
        return;
      }
      ...
      const area = chartCells({ cols, rows }, { left: labelCols + 1, top: 1, bottom: 1 });
      const axisRow = rows - 1;
      const x = (time) => area.colAt((time - timeMin) / timeSpan);
      const perRow = area.rows >= data.length * 3 - 1 ? 2 : 1;
      const rowGap = area.rows >= data.length * (perRow + 1) - 1 ? 1 : 0;
      const stride = perRow + rowGap;
      const bodyBottom = Math.min(area.row + area.rows - 1, area.row + data.length * stride - rowGap - 1);
...
(function () {
  ...
  var instance = PicaTimelineChart.mount(host, take(window.PICA_PROPS || {}));
