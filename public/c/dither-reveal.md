...
> A photograph that dissolves in and out as a halftone that thickens in blue noise order.

Category: dither. Tags: image, reveal, dither, canvas, halftone. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 5.1 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-reveal.json
```
...
| `mask` | "blue" \| "bayer" | `"blue"` | "blue" reveals in an even grain from a void-and-cluster mask. "bayer" reveals in a visible crosshatch. |
| `scale` | number | `3` | CSS pixels per dithered dot. Higher values give a coarser, more graphic result. |
| `reveal` | number | `1800` | Milliseconds the picture takes to dissolve fully in, and again to dissolve fully out. |
| `hold` | number | `3600` | Milliseconds the finished picture holds before it dissolves out again. |
...
// Pica · Dither Reveal · dither-reveal
...
// registry/dither/dither-reveal/core.ts
...
// registry/dither/dither-reveal/index.tsx
...
<title>Dither Reveal · Pica</title>
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          load();
        } else if (recolored || props.fit !== before.fit || props.tone !== before.tone || props.contrast !== before.contrast || props.mask !== before.mask || props.scale !== before.scale) {
          prepare();
        } else {
          loop.redraw();
        }
...
(function () {
  ...
  var instance = PicaDitherReveal.mount(host, take(window.PICA_PROPS || {}));
