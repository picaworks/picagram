...
Category: text-mode. Tags: frame, border, box-drawing, static. Static. Size: 2.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/pica/blob/main/LICENSE.md.
...
| `accent` | boolean | `false` | Draws the title in --pica-accent instead of the border's own ink. |
...
var PicaAsciiFrame = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    function draw() {
      ...
      const [left, label, right] = topRule(cols, chars, props.title, props.titleAlign);
      grid.write(0, 0, left);
      grid.write(left.length, 0, label, props.accent ? accentInk(host) : void 0);
      grid.write(left.length + label.length, 0, right);
      if (rows > 1) grid.write(0, rows - 1, chars.bl + chars.h.repeat(Math.max(0, cols - 2)) + chars.br);
      for (let y = 1; y < rows - 1; y++) {
        grid.set(0, y, chars.v);
...
(function () {
  var instance = PicaAsciiFrame.mount(document.getElementById("pica"), window.PICA_PROPS || {});
