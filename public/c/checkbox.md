...
> A hairline checkbox with checked, unchecked, and mixed states for pointer and keyboard input.

Category: ui. Tags: checkbox, form, selection, ui. Static. Size: 1.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/checkbox.json
```
...
|---|---|---|---|
| `checked` | boolean \| "mixed" \| null | `null` | The controlled state. Null uses the state read from defaultChecked at mount. |
| `defaultChecked` | boolean \| "mixed" | `true` | The initial state when checked is null. |
| `label` | string | `"Include archived builds"` | Text shown beside the box and used as its accessible name. |
| `disabled` | boolean | `false` | Blocks input and dims the checkbox. |
...
|---|---|---|---|
| `checkedChange` | `onCheckedChange` | `"checked" \| "unchecked" \| "mixed"` | The state requested by pointer or keyboard input. |
...
// registry/ui/checkbox/index.tsx
...
var PicaCheckbox = (() => {
  ...
  function checkboxRules(selector) {
    const fg = cssVar("fg");
    const accent = cssVar("accent");
    return [
      `${selector}{appearance:none;margin:0;padding:0;border:0;background:transparent;color:${fg};font:inherit;line-height:1.2;display:inline-block;white-space:nowrap;cursor:pointer;user-select:none;outline:none}`,
  ...
  function controlledState(value) {
    if (value === "checked") return true;
    if (value === "unchecked") return false;
    return value;
  }
  var mount = (host, initial = {}) => {
    ...
    let local = props.defaultChecked;
    ...
    box.append(mark);
    host.append(box, label);
    const state = () => controlledState(props.checked) ?? local;
    function draw() {
      const current = state();
      attrs.set("aria-checked", ariaState(current));
      mark.textContent = current === "mixed" ? "─" : current ? "✓" : "";
...
(function () {
  ...
  var instance = PicaCheckbox.mount(host, take(window.PICA_PROPS || {}));
  ["checkedChange"].forEach(function (name) {
