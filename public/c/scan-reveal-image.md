...
Category: effects. Tags: image, scan, fax, reveal, canvas. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.6 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/scan-reveal-image.json
```
...
| `duration` | number | `5000` | Milliseconds one sweep takes, from the top edge to the bottom. |
| `hold` | number | `2000` | Milliseconds the finished picture holds at the end of a sweep, before the next one starts. |
| `bar` | number | `2` | The scan bar's thickness in pixels. The bar is the one mark drawn in the accent. |
| `preview` | number | `0.25` | How much ink the sparse dither keeps where the scan has not reached yet, 0 to 1. |
...
// Pica · Scan Reveal Image · scan-reveal-image
...
// registry/effects/scan-reveal-image/core.ts
...
// registry/effects/scan-reveal-image/index.tsx
...
<title>Scan Reveal Image · Pica</title>
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          load();
        } else if (recolored || props.fit !== before.fit || props.tone !== before.tone || props.contrast !== before.contrast || props.preview !== before.preview) {
          prepare();
        } else {
          loop.redraw();
        }
...
(function () {
  ...
  var instance = PicaScanRevealImage.mount(host, take(window.PICA_PROPS || {}));
