...
> A radar plan position display whose accent beam sweeps hairline rings and spokes, lighting seeded echoes that decay in dithered steps.

Category: shaders. Tags: radar, sweep, shader, webgl, dither. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 5.0 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/scan-beam.json
```
...
|---|---|---|---|
| `period` | number | `8` | Seconds per full turn of the beam. |
| `rings` | number | `4` | Range rings drawn as hairlines, counting the outer rim. |
| `spokes` | number | `12` | Bearing spokes radiating from the centre. |
| `echoes` | number | `7` | Seeded echoes that light as the beam crosses them, then fade in steps. |
| `persistence` | number | `0.7` | How far around the circle a return keeps showing, from 0 (the beam alone) to 1 (the whole turn). |
| `levels` | number | `6` | Tone steps the decay is dithered between: 2 is one-bit, 16 reads as nearly smooth. |
...
// registry/shaders/scan-beam/core.ts
...
// registry/shaders/scan-beam/index.tsx
...
(function () {
  ...
  var instance = PicaScanBeam.mount(host, take(window.PICA_PROPS || {}));
