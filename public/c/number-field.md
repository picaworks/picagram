...
> A labelled numeric input with typed entry, keyboard stepping, and square decrement and increment controls.

Category: ui. Tags: number, spinbutton, input, form, ui. Static. Size: 2.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/number-field.json
```
...
|---|---|---|---|
| `value` | number \| null | `null` | The current number. Null lets the field manage its own value. |
| `defaultValue` | number | `8` | The number shown initially when value is null. |
| `min` | number | `0` | The smallest value the field accepts. |
| `max` | number | `64` | The largest value the field accepts. |
| `step` | number | `1` | The amount added or subtracted by the arrow keys and stepper buttons. |
| `largeStep` | number | `10` | The amount added or subtracted by Page Up and Page Down. |
| `label` | string | `"Columns"` | The visible name of the field. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `number` | The user committed a different number. |
...
// Pica · Number Field · number-field
...
// registry/ui/number-field/core.ts
...
// registry/ui/number-field/index.tsx
...
var PicaNumberField = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    const onKeyDown = (event) => {
      ...
      event.preventDefault();
      if (proposed === null && edge === void 0) paint();
      else commit(proposed ?? current, edge);
    };
    const onBlur = () => commitTyped();
    const onDecrement = () => commit(current - stepSize(props.step));
    const onIncrement = () => commit(current + stepSize(props.step));
    input.addEventListener("keydown", onKeyDown);
    input.addEventListener("blur", onBlur);
    decrement.addEventListener("click", onDecrement);
    increment.addEventListener("click", onIncrement);
...
(function () {
  ...
  var instance = PicaNumberField.mount(host, take(window.PICA_PROPS || {}));
