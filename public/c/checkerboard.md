...
> A quiet field of alternating ink and transparent squares drawn beneath page content.

Category: patterns. Tags: checkerboard, squares, grid, css. Static. Size: 1.4 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/checkerboard.json
```
...
|---|---|---|---|
| `size` | number | `32` | Width and height of each square, in pixels. |
| `angle` | number | `0` | Clockwise tilt of the board, in degrees. |
| `strength` | number | `0.08` | Opacity of the inked squares. |
...
// Pica · Checkerboard · checkerboard
...
// registry/patterns/checkerboard/core.ts
...
// registry/patterns/checkerboard/index.tsx
...
<body>
<div id="pica"><div><h2>Quiet geometry</h2><p>A measured field leaves every word clear.</p></div></div>
<script>
"use strict";
var PicaCheckerboard = (() => {
  ...
  function rules(selector, cell, strength) {
    const ink = cssVar("fg");
    const tile = cell * 2;
    return `${selector}>div[data-pica]{color:${ink};background-image:repeating-conic-gradient(currentColor 0 25%,transparent 0 50%);background-size:${tile}px ${tile}px;opacity:${strength}}`;
  }
...
(function () {
  ...
  var instance = PicaCheckerboard.mount(host, take(window.PICA_PROPS || {}));
