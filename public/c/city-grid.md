...
> A seeded city of blocks on a street grid, seen from above at a fixed tilt and panning slowly past.

Category: immersive. Tags: city, axonometric, skyline, procedural, background. Animated. Holds a still frame under prefers-reduced-motion, and stops offscreen and in hidden tabs. Size: 4.7 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/city-grid.json
```
...
|---|---|---|---|
| `density` | number | `0.7` | Share of blocks that carry a building, from 0 to 1. The rest stay empty lots. |
| `blockSize` | number | `18` | The street grid's module on screen, in CSS pixels. A block spans a few modules on each side. |
| `height` | number | `0.5` | Height of the skyline from 0 to 1. 0 leaves a city of one-storey blocks. |
| `spread` | number | `1.5` | How wide each cluster of tall blocks grows. Larger spreads reach further past the downtown. |
| `angle` | number | `30` | Tilt of the axonometric view in degrees. A low angle looks across the roofs. |
| `speed` | number | `0.12` | Pan speed along one street axis. 0 holds the city still. |
| `fps` | number | `20` | Frames per second ceiling. |
...
// registry/immersive/city-grid/index.tsx
