...
> An image posterized into flat tone bands whose step edges dither into narrow stippled transitions.

Category: dither. Tags: image, static, poster, dither, canvas. Static. Size: 4.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/dither-poster-image.json
```
...
| `levels` | number | `4` | Flat tone bands the image posterizes into. |
| `band` | number | `0.12` | Width of the dithered transition at each step, as a share of the tone range. 0 is a hard edge. |
| `mask` | "bayer" \| "blue" | `"bayer"` | The ordered mask that screens a transition: a Bayer matrix, or a blue noise pattern. |
| `scale` | number | `2` | CSS pixels per posterized pixel. Higher values give a coarser, more graphic result. |
...
// Pica · Dither Poster Image · dither-poster-image
...
// registry/dither/dither-poster-image/core.ts
...
// registry/dither/dither-poster-image/index.tsx
...
<title>Dither Poster Image · Pica</title>
...
var PicaDitherPosterImage = (() => {
  ...
  var BAYER_SIZE = 8;
  var BLUE_SIZE = 32;
  function bandAt(t, levels, band, mask) {
    const top = levels - 1;
    const shifted = (t - (mask - 0.5) * band) * levels;
    if (shifted <= 0) return 0;
    return shifted >= levels ? top : Math.floor(shifted);
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        ...
        if (props.src !== before.src) {
          load();
        } else if (recolored || props.fit !== before.fit || props.tone !== before.tone || props.contrast !== before.contrast || props.levels !== before.levels || props.band !== before.band || props.mask !== before.mask || props.scale !== before.scale) {
          prepare();
        } else {
          paint();
        }
...
(function () {
  ...
  var instance = PicaDitherPosterImage.mount(host, take(window.PICA_PROPS || {}));
