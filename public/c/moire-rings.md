...
> Two concentric hairline ring fields drift out of step to form a quiet radial moire behind content.

Category: patterns. Tags: moire, rings, radial, interference, css. Static. Size: 1.3 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/moire-rings.json
```
...
|---|---|---|---|
| `spacing` | number | `7` | Distance from one ring to the next in the first field, in pixels. |
| `step` | number | `1.07` | Period of the second field as a fraction of the first field. |
| `thickness` | number | `1` | Width of each ring, in pixels. |
| `strength` | number | `0.3` | Opacity of the complete ring field. |
...
// registry/patterns/moire-rings/core.ts
...
// registry/patterns/moire-rings/index.tsx
...
<body>
<div id="pica"><h2>Interference field</h2><p>Two ring systems resolve into a slow radial beat.</p></div>
<script>
"use strict";
var PicaMoireRings = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    return {
      update(next) {
        const before = props;
        props = { ...props, ...next };
        if (props.spacing !== before.spacing || props.step !== before.step || props.thickness !== before.thickness || props.strength !== before.strength) {
          sheet.setRules(moireRules(sheet.selector, props));
        }
      },
      destroy() {
  ...
  function moireRules(selector, props) {
    const spacing = Math.min(16, Math.max(4, props.spacing));
    const step = Math.min(1.25, Math.max(1.02, props.step));
    const thickness = Math.min(2, Math.max(1, props.thickness));
    const strength = Math.min(1, Math.max(0, props.strength));
...
(function () {
  ...
  var instance = PicaMoireRings.mount(host, take(window.PICA_PROPS || {}));
