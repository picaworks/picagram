...
> A quiet masonry lattice with running or stacked courses drawn behind content.

Category: patterns. Tags: brick, masonry, lattice, background, css. Static. Size: 1.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/brick-lattice.json
```
...
|---|---|---|---|
| `bond` | "running" \| "stack" | `"running"` | Course bond, either half pitch offset or vertically aligned. |
| `width` | number | `48` | Width of each brick, in pixels. |
| `course` | number | `16` | Height of each masonry course, in pixels. |
| `mortar` | number | `3` | Ground left between adjacent bricks, in pixels. |
| `strength` | number | `0.35` | Opacity of the lattice, from quiet to fully visible. |
...
// Pica · Brick Lattice · brick-lattice
...
// registry/patterns/brick-lattice/core.ts
...
// registry/patterns/brick-lattice/index.tsx
...
<title>Brick Lattice · Pica</title>
...
<body>
<div id="pica"><h2>Built in courses</h2><p>A measured pattern gives the page quiet structure.</p></div>
<script>
"use strict";
var PicaBrickLattice = (() => {
  ...
  function brickLatticeRules(selector, p) {
    const pitch = p.width + p.mortar;
    const rise = p.course + p.mortar;
    const tileWidth = pitch * 2;
    const tileHeight = rise * 2;
  ...
  function brickLatticeSvg(p, pitch, rise, tileWidth, tileHeight) {
    const offset = p.bond === "running" ? pitch / 2 : 0;
...
(function () {
  ...
  var instance = PicaBrickLattice.mount(host, take(window.PICA_PROPS || {}));
