...
> An image split into a dark layer and a light layer, each posterized flat and slid apart along one axis.

Category: effects. Tags: image, static, posterize, canvas, registration. Static. Size: 3.8 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/channel-shift-image.json
```
...
| `offset` | number | `8` | How far the light layer slides from the dark one, in CSS pixels. |
| `angle` | number | `0` | Degrees clockwise from horizontal that the light layer slides in: 0 moves it right, 90 moves it down. |
| `split` | number | `0.5` | The ink value that splits the dark layer from the light one, 0 to 1. |
| `accent` | boolean | `true` | Draws the light layer in --pica-accent. Off draws both layers in the host's ink color, at two opacities. |
| `overlap` | "darken" \| "over" | `"darken"` | How the layers land where they overlap: "darken" lets their ink accumulate into a third, darker area; "over" lets the light layer simply cover the dark one. |
...
// Pica · Channel Shift Image · channel-shift-image
...
// registry/effects/channel-shift-image/core.ts
...
// registry/effects/channel-shift-image/index.tsx
...
<title>Channel Shift Image · Pica</title>
...
var PicaChannelShiftImage = (() => {
  ...
  var GHOST_MIX = 0.55;
  var OVERLAP_DARKEN = 0.7;
  function fadeColor(color, factor) {
    return [color[0], color[1], color[2], Math.round(color[3] * factor)];
  }
  function darkenBlend(a, b) {
    const red = Math.round((a[0] + b[0]) / 2 * OVERLAP_DARKEN);
    const green = Math.round((a[1] + b[1]) / 2 * OVERLAP_DARKEN);
    const blue = Math.round((a[2] + b[2]) / 2 * OVERLAP_DARKEN);
    return [red, green, blue, Math.max(a[3], b[3])];
  ...
  function splitLayers(ink, coverage, split) {
    const dark = new Uint8Array(ink.length);
    const light = new Uint8Array(ink.length);
    for (let i = 0; i < ink.length; i++) {
      if ((coverage[i] ?? 0) < 0.5) continue;
