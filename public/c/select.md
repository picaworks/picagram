...
|---|---|---|---|
| `options` | readonly SelectOption[] | `[{"value":"ascii","label":"ASCII","disabled":false},{"value":"dither","label":"Dither","disabled":false},{"value":"shaders","label":"Shaders","disabled":false},{"value":"charts","label":"Charts","disabled":false}]` | The choices offered, in order. |
| `value` | string \| null | `null` | The chosen value. Null means uncontrolled, so the component tracks its own choice. |
| `defaultValue` | string | `"ascii"` | The value chosen at mount, read once, while value is null. |
| `placeholder` | string | `"Choose one"` | Shown in the trigger when nothing is chosen. |
| `label` | string | `"Family"` | The accessible name for the control. An empty label hides it. |
...
|---|---|---|---|
| `valueChange` | `onValueChange` | `string` | The value of the option the user chose. |
...
var PicaSelect = (() => {
  ...
  var mount = (host, initial = {}) => {
    ...
    let current = props.value ?? props.defaultValue;
    let lastOptions;
    let rows = [];
    let activeIndex = -1;
    let openState = false;
    ...
    function setActive(index) {
      if (index === activeIndex) return;
      const prev = activeIndex >= 0 ? rows[activeIndex] : void 0;
      prev?.removeAttribute("data-active");
      prev?.setAttribute("aria-selected", "false");
      activeIndex = index;
      const row = activeIndex >= 0 ? rows[activeIndex] : void 0;
      if (row) {
        row.setAttribute("data-active", "true");
        row.setAttribute("aria-selected", "true");
        attrs.set("aria-activedescendant", row.id);
    ...
    function setOpen(next, options) {
      if (openState === next) {
        if (next && options?.activeIndex !== void 0) setActive(options.activeIndex);
        return;
      }
      openState = next;
      if (next) {
    ...
    function onHostFocusOut(e) {
      if (host.contains(e.relatedTarget)) return;
      if (openState) setOpen(false);
    }
    function onHostPointerOver(e) {
      if (!openState || props.disabled) return;
      const row = e.target.closest?.("[data-pica-option]");
      if (!row || !listboxEl.contains(row)) return;
      const index = Number(row.dataset.picaIndex);
      if (Number.isNaN(index) || props.options[index]?.disabled) return;
      setActive(index);
    ...
    function onHostClick(e) {
      ...
      const row = target.closest("[data-pica-option]");
      if (row && listboxEl.contains(row)) {
        const index = Number(row.dataset.picaIndex);
        if (!Number.isNaN(index)) choose(index);
        return;
      ...
      if (listboxEl.contains(target)) return;
      setOpen(!openState, openState ? void 0 : { activeIndex: defaultActiveIndex() });
    }
    ...
    function apply() {
      sheet.setRules(rules(sheet.selector, anchorVar));
      if (lastOptions === void 0 || !sameJson(props.options, lastOptions)) {
        lastOptions = props.options;
        renderOptions();
        activeIndex = -1;
        if (openState) setActive(defaultActiveIndex());
