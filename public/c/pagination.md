...
> A compact navigation list that keeps nearby pages visible and folds distant ranges into quiet ellipses.

Category: ui. Tags: pagination, navigation, pages, ui. Static. Size: 2.1 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/pagination.json
```
...
|---|---|---|---|
| `page` | number \| null | `null` | The current page. Null lets the component manage its own page. |
| `defaultPage` | number | `4` | The initial page when page is null. It is read once when the component mounts. |
| `pages` | number | `9` | The total number of pages, from 2 to 99. |
| `siblings` | number | `1` | The number of pages kept on each side of the current page, from 0 to 3. |
| `base` | string | `""` | A URL prefix joined to each destination page number. An empty prefix keeps navigation in place. |
| `label` | string | `"Pages"` | The accessible name of the pagination navigation. |
...
|---|---|---|---|
| `pageChange` | `onPageChange` | `number` | A page link was activated. |
...
// registry/ui/pagination/core.ts
...
// registry/ui/pagination/index.tsx
...
var PicaPagination = (() => {
  ...
  function paginationRange(value, minimum, maximum) {
    return Math.min(maximum, Math.max(minimum, Math.round(value)));
  }
  function paginationItems(page, pages, siblings) {
    const visible = /* @__PURE__ */ new Set([1, pages]);
    for (let value = page - siblings; value <= page + siblings; value++) {
      if (value > 1 && value < pages) visible.add(value);
    }
    const ordered = [...visible].sort((a, b) => a - b);
  ...
  function paginationRules(selector) {
    const fg = cssVar("fg");
    const muted = cssVar("muted");
    const accent = cssVar("accent");
    return [
  ...
  var mount = (host, initial = {}) => {
    ...
    host.append(nav);
    sheet.setRules(paginationRules(sheet.selector));
    function destinationLink(text, page, name, disabled = false, icon = "") {
      const link = paginationNode("a");
      link.setAttribute("data-pica-link", "");
      link.setAttribute("data-pica-page", String(page));
      link.href = `${props.base}${page}`;
    ...
    const onClick = (event) => {
      const target = event.target instanceof Element ? event.target.closest("a[data-pica-page]") : null;
      if (!target || !nav.contains(target)) return;
      if (target.getAttribute("aria-disabled") === "true") {
        event.preventDefault();
        return;
...
(function () {
  ...
  var instance = PicaPagination.mount(host, take(window.PICA_PROPS || {}));
  ["pageChange"].forEach(function (name) {
