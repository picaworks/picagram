...
Category: data. Tags: chart, gauge, arc, svg, glyph grid, data table. Static. Size: 5.5 KB gzipped, runtime included. License: MIT + Commons Clause, https://github.com/rishabbalak/picagram/blob/main/LICENSE.md.

## Install

```bash
npx shadcn@latest add https://picagram.dev/r/gauge-chart.json
```
...
|---|---|---|---|
| `value` | number | `68` | The current value to display. |
| `min` | number | `0` | The minimum value of the range. |
| `max` | number | `100` | The maximum value of the range. |
| `label` | string | `"Cache hit rate"` | Name for the gauge: drawn under the value, read by assistive technology, and used as the hidden data table's caption. |
| `unit` | string | `"%"` | Unit to display after the value (e.g., "%", "°C"). |
| `thresholds` | number[] | `[50,90]` | Threshold values to mark on the arc as ticks. |
| `thickness` | number | `0.16` | Ring thickness as a fraction of its radius (0.05 to 0.4). |
| `look` | "svg" \| "glyph" | `"svg"` | "svg" draws an SVG arc with labels. "glyph" draws the same arc in a braille grid. |
...
// registry/data/gauge-chart/index.tsx
...
var PicaGaugeChart = (() => {
  ...
  var ARC_START = 240 * Math.PI / 180;
  var ARC_SWEEP = 240 * Math.PI / 180;
  var mount = (host, initial = {}) => {
    ...
    function tickValues() {
      const span = props.max - props.min;
      return Array.from(/* @__PURE__ */ new Set([props.min, props.max, ...props.thresholds])).filter((t) => {
        const share = (t - props.min) / span;
        return share >= 0 && share <= 1;
      });
    }
    ...
    function drawGlyph() {
      ...
      const margin = Math.max(16, parseFloat(getComputedStyle(host).fontSize) || 16);
      const inset = {
        top: Math.ceil(margin / g.cellHeight),
        bottom: Math.ceil(margin / g.cellHeight),
        left: Math.ceil(margin / g.cellWidth),
        right: Math.ceil(margin / g.cellWidth)
      };
      const area = chartCells({ cols, rows }, inset);
      const track = chartDots(area, g.aspect);
      const fill = chartDots(area, g.aspect);
...
(function () {
  ...
  var instance = PicaGaugeChart.mount(host, take(window.PICA_PROPS || {}));
