...
| `label` | string | `"Cache hit rate"` | Name for the gauge, read by assistive technology and used as the hidden data table's caption. |
| `unit` | string | `"%"` | Unit to display after the value (e.g., "%", "°C"). |
| `thresholds` | number[] | `[50,90]` | Threshold values to mark on the arc as ticks. |
| `thickness` | number | `0.16` | Thickness of the arc as a fraction of the radius (0.05 to 0.4). |
